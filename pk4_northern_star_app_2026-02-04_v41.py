...
def _normalize_core(x):
    x = str(x).strip()
    x = "".join(ch for ch in x if ch.isdigit())
    return x.zfill(3) if x else ""

def _get_available_cores(df_all):
    cores = set(_normalize_core(c) for c in CORE_PRESETS if str(c).strip())
    if df_all is not None and len(getattr(df_all, "columns", [])) > 0:
        for col in ("Core", "core", "core_id", "coreID"):
            if col in df_all.columns:
                for v in df_all[col].dropna().unique().tolist():
                    nv = _normalize_core(v)
                    if nv:
                        cores.add(nv)
    cores = sorted([c for c in cores if c])
    # Always keep a sane fallback so the app never boots with an empty list
    return cores if cores else ["389"]

available_cores = _get_available_cores(df_all)

# View core dropdown (single core)
_default_view_core = _normalize_core(st.session_state.get("view_core") or st.session_state.get("core_default") or "389")
_view_index = available_cores.index(_default_view_core) if _default_view_core in available_cores else 0
view_core = st.selectbox("View core (dropdown)", options=available_cores, index=_view_index, key="view_core")

# Multi-core selection for cache building / batch tools
_prior = st.session_state.get("cores_for_cache", [view_core])
if isinstance(_prior, str):
    _prior = [_prior]
_prior = [_normalize_core(c) for c in (_prior or []) if _normalize_core(c)]
# ensure the current view_core is always included
if view_core not in _prior:
    _prior.insert(0, view_core)
_prior = [c for c in _prior if c in available_cores]
...
# Never allow the multi-core selection to be empty (keeps downstream logic stable)
if not cores_for_cache:
    cores_for_cache = [view_core]
    st.session_state["cores_for_cache"] = cores_for_cache

# Optional: show tabs for all selected cores
show_tabs_all_cores = st.checkbox("Show tabs for all selected cores (can be slower)", value=False)

# For compatibility with the rest of the script (older variable name)
cores = cores_for_cache
