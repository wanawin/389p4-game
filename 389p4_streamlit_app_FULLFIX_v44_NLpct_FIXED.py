...
def build_northern_star_buckets(stream_stats_df: pd.DataFrame, cfg: "RankConfig"):
    """Compatibility helper for Core View.

    Returns three lists: (Top 12, Due 8, Combined) stream names.
    """
    b = bucket_recommendations(stream_stats_df, cfg)
    return b.get("base_top", []), b.get("due_top", []), b.get("combined", [])
...
# --- safety: initialize core lists early to avoid NameError on first load ---
cores: list[str] = []
cores_for_cache: list[str] = []
...
def build_northern_star_bucket_meta_v1(stats_df: pd.DataFrame, stream: str,
    top_n: int,
    ...
    if not cores_for_cache:
        ...
        for core in cores_for_cache:
            ...
            for stream in stats_df["Stream"].astype(str).tolist():
                meta = build_northern_star_bucket_meta(
                    stats_df=stats_df,
                    stream=stream,
                    top_n=cfg.top_base,
                ...
                try:
                    ...
                    if pos_df is not None and not pos_df.empty:
                        # position map is keyed by RankPos; use this stream's RankPos as lookup
                        try:
                            rp = int(stats_df.loc[stats_df["Stream"].astype(str) == str(stream), "RankPos"].iloc[0])
                            m = pos_df[pos_df["RankPos"].astype(int) == rp]
                            if not m.empty:
                                p = float(m.iloc[0].get("HitCountPctile", 0.0) or 0.0)
                        except Exception:
                            p = 0.0
