...
# (Prevents NameError / signature-mismatch crashes across file versions.)
# ---------------------------------------------------------------------
_T_NULL = nullcontext()
# Safe tab placeholders (overwritten once tabs are created).
_t_ns = _t_nl = _t_core = _t_bt = _T_NULL

APP_VERSION = "v51.08 (SeedTraits+Cadence+AllCoresCacheOnly)"
...
def evaluate_rare_engine(
    ...
    **kwargs: object,
    ...
    # --- argument normalization (supports older call patterns) ---
    # Legacy versions called: evaluate_rare_engine(core, df_all, window_days)
    if isinstance(df_all, str) and hasattr(core, 'columns'):
        legacy_core = df_all
        legacy_df_all = core
        # In the legacy 3-arg call, the 3rd arg (window_days) lands in df_24h as an int.
        if isinstance(df_24h, (int, float)):
            window_days_recent = int(df_24h)
            df_24h = None
        df_all = legacy_df_all
        core = legacy_core
    # accept alternate kwarg names
    if isinstance(kwargs.get('window_days'), (int, float)):
        window_days_recent = int(kwargs.pop('window_days'))
    if 'enable_ur' in kwargs:
        enable_r4 = bool(kwargs.pop('enable_ur'))
    enable_r1 = bool(kwargs.pop('enable_r1', enable_r1))
    enable_r2 = bool(kwargs.pop('enable_r2', enable_r2))
    enable_r3 = bool(kwargs.pop('enable_r3', enable_r3))
    enable_r4 = bool(kwargs.pop('enable_r4', enable_r4))
    # ignore any extra legacy args/kwargs quietly
...
try:
    ...
    # overwrite the safe placeholders defined near the imports
    _t_ns = tabs[0]
    _t_nl = tabs[1]
    _t_core = tabs[2]
    _t_bt = tabs[3]
except Exception:
    # If tabs are unavailable for any reason, fall back to writing in the main page.
    _t_ns = _t_nl = _t_core = _t_bt = _T_NULL
