...
APP_VERSION = "v51.2 (build 2026-02-08 20:30 CST)  [FIX: pctmap & baseline unpack]"
...
def build_allcores_rankpos_pctmap(
    ...
    for core in cores_list:
        if cache_only:
            ss, _ld, _view = _load_baseline_from_disk(core, window_days, expected_last_date=expected_last_date)
    ...
    if not cores_for_cache:
        ...
        if nl_use_all_cores:
            ...
            for _c in cores_for_cache:
                ss, _meta, _view = _load_baseline_from_disk(_c, cfg.window_days, expected_last_date=expected_last)
                if ss is None or ss.empty:
                    missing.append(_c)
                else:
                    stats_by_core[_c] = ss
