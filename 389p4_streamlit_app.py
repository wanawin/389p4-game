...
# Persist selection
# ---- Multi-core cache selection (safe session_state; no StreamlitAPIException) ----
# Selecting a core in the dropdown should add it to the cache/batch list.
# IMPORTANT: never write to a widget-backed session_state key *after* the widget is created.

# Initialize once
if 'cores_for_cache' not in st.session_state or not st.session_state.get('cores_for_cache'):
    st.session_state['cores_for_cache'] = [default_core]

def _on_view_core_change():
    vc = str(st.session_state.get('view_core', default_core)).zfill(3)
    cur = [str(c).zfill(3) for c in (st.session_state.get('cores_for_cache') or [])]
    if not cur:
        cur = [default_core]
    if vc not in cur:
        st.session_state['cores_for_cache'] = cur + [vc]

def _ensure_nonempty_cache_cores():
    if not st.session_state.get('cores_for_cache'):
        st.session_state['cores_for_cache'] = [default_core]

# Core view dropdown (single core)
view_core = st.selectbox(
    "View core (dropdown)",
    options=available_cores,
    index=available_cores.index(default_core) if default_core in available_cores else 0,
    key='view_core',
    on_change=_on_view_core_change,
)
...
st.multiselect(
    ...
    options=available_cores,
    default=st.session_state.get('cores_for_cache', [default_core]),
    key='cores_for_cache',
    on_change=_ensure_nonempty_cache_cores,
)

cores_for_cache = [str(c).zfill(3) for c in (st.session_state.get('cores_for_cache') or [default_core])]
...
cores_for_tabs = st.session_state.get("cores_for_cache", ["389"])
cores_for_tabs = [str(c).zfill(3) for c in (cores_for_tabs or [])]
if not cores_for_tabs:
    cores_for_tabs = ["389"]
tab_labels = ["Northern Lights (Master playlist)", "Core view", "Backtest (optional)"] + [f"Core {c}" for c in cores_for_tabs]
