...
APP_VERSION = "v42+NLpct+CorePct+CoreTabs"
...
# NOTE: This uses the SAME session_state key as the widget to avoid StreamlitAPIException.
cores_for_cache = st.multiselect(
    "Cores to include for cache building / batch tools",
    options=available_cores,
    default=list(st.session_state.cores_for_cache),
    key="cores_for_cache",
)

# Keep at least one core selected
if not st.session_state.cores_for_cache:
    st.session_state.cores_for_cache = [default_core]
...
with tabs[1]:
    ...
    if df_all is None or df_all.empty:
        st.info("Upload a history file to view per-core rankings.")
                ...
                show_tabs_all_cores = st.checkbox(
            "Show tabs for all selected cores",
            value=False,
            key="core_view_show_tabs",
        )

        def _render_one_core(core_id: str):
            core_for_view = core_id
        ...
        if show_tabs_all_cores:
            core_list = list(st.session_state.cores_for_cache) or [default_core]
            core_tabs = st.tabs([f"Core {c}" for c in core_list])
            for _i, _c in enumerate(core_list):
                with core_tabs[_i]:
                    _render_one_core(_c)
        else:
            core_for_view_pick = st.selectbox("Choose a core", cores, index=0, key="core_view_pick")
            _render_one_core(core_for_view_pick)
with tabs[2]:
