...
def compute_stream_stats(df: pd.DataFrame,
    ...
    # Some history parsers only provide (date,state,game,result). If the family
    # column is absent, derive it from the 4-digit result using the existing
    # box_key() helper so downstream logic matches the original behavior.
    if "family" not in df.columns:
        df = df.copy()
        df["family"] = df["result"].astype(str).apply(box_key)
    ...
    for (state, game), g in groups:
        ...
        # Family shares
        # Extra safety: if an upstream selection dropped the derived column,
        # reconstruct it from the 4-digit result so this section never crashes.
        fam_series = g["family"] if "family" in g.columns else g["result"].astype(str).apply(box_key)
        counts_by_family = fam_series.value_counts().to_dict()
...
with st.sidebar:
    ...
    # Defaults to prevent NameError when the optional schedule UI block is skipped
    schedule_mode = "Multiply (strong schedule influence)"
...
schedule_laplace_alpha = 2.0 if "schedule_laplace_alpha" not in locals() or schedule_laplace_alpha is None else schedule_laplace_alpha
