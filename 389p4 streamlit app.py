...
def build_allcores_rankpos_pctmap(
    ...
    """Aggregate RankPos->HitCount across many cores and return a percentile map + missing list.

    Return:
      - pct_map_df (or None if nothing to build)
      - missing_cores: any cores missing/outdated baseline when cache_only=True

    Notes:
      - If cache_only=True, we do NOT recompute; we only load from disk baseline caches.
        Missing cores are reported instead of crashing.
      - If cache_only=False, uses compute_stream_stats_for_core(...) with the provided dataframes.
    """
...
with _t_nl:
    ...
    # Safety defaults to prevent NameError across tabs / reruns (Streamlit can rerun from any widget interaction)
    _sel = (
        st.session_state.get("cores_for_cache_ms")
        or st.session_state.get("cores_for_cache")
        or st.session_state.get("selected_cores")
        or []
    )
    if isinstance(_sel, (set, tuple)):
        selected_cores = list(_sel)
    elif isinstance(_sel, list):
        selected_cores = _sel
    elif _sel is None:
        selected_cores = []
    else:
        selected_cores = [str(_sel)]

    if not selected_cores and isinstance(view_core, str) and view_core:
        selected_cores = [view_core]
    ...
    if stats_ns is not None and not stats_ns.empty:
        ...
        if st.session_state.get("r1", True) or st.session_state.get("r2", True) or st.session_state.get("r3", True) or st.session_state.get("r4", True):
            r1v = bool(st.session_state.get("r1", True))
            r2v = bool(st.session_state.get("r2", True))
            r3v = bool(st.session_state.get("r3", True))
            r4v = bool(st.session_state.get("r4", True))
            rare_df, rare_summary = evaluate_rare_engine(df_all, core_key_ns, df_24h, r1v, r2v, r3v, r4v, cfg.window_days)
            st.dataframe(rare_df, use_container_width=True)
            with st.expander("Rare Engine summary", expanded=False):
                st.json(rare_summary)
        else:
        ...
        if st.session_state.get("q1", True) or st.session_state.get("q2", True) or st.session_state.get("q3", True) or st.session_state.get("q4", True):
            q1v = bool(st.session_state.get("q1", True))
            q2v = bool(st.session_state.get("q2", True))
            q3v = bool(st.session_state.get("q3", True))
            q4v = bool(st.session_state.get("q4", True))
            ultra_df, ultra_summary = evaluate_ultra_rare_engine(df_all, core_key_ns, df_24h, q1v, q2v, q3v, q4v, cfg.window_days)
            st.dataframe(ultra_df, use_container_width=True)
            with st.expander("Ultra-Rare Engine summary", expanded=False):
                st.json(ultra_summary)
        else:
