...
def extract_4digit(x: str) -> Optional[str]:
    """Extract a clean 4-digit string from various formats (alias helper)."""
    return extract_pick4_digits(x)
...
def compute_stream_stats(df_all: pd.DataFrame, core: str, window_days: int, exclude_md: bool = False) -> pd.DataFrame:
    """Compute per-stream stats for a given 3-digit core (AABC family members) within a rolling window."""
    if df_all is None or df_all.empty:
        return pd.DataFrame(columns=["Stream","DrawsWindow","HitsWindow","HitsPerWeek","LastHitDate","DaysSinceLastHit","RankPos"])
    ...
    try:
        members = members_from_core(core_key, "AABC")
    except Exception:
        members = []

    # hits = any member of this core-family
    df_hits = df_all[df_all.get("Box", pd.Series(dtype=str)).isin(members)].copy() if members else df_all.iloc[0:0].copy()

    # Note: exclude_md is already applied to df_all upstream in the UI; keep param for signature compatibility.
    return stream_summary(df_all=df_all, df_hits=df_hits, window_days=int(window_days))
...
def build_northern_star_buckets(
    ...
    """Return a compact bucket summary dict for a (core, stream) row."""
    if stats_df is None or stats_df.empty:
        return {"Top12": "Other", "BucketPick": "", "SeedKey": "", "DuePressure": 0.0, "HitsPerWeek": 0.0, "Hits": 0, "DaysSinceLastHit": 0}

    srow = stats_df[stats_df["Stream"] == stream]
    if srow.empty:
        return {"Top12": "Other", "BucketPick": "", "SeedKey": "", "DuePressure": 0.0, "HitsPerWeek": 0.0, "Hits": 0, "DaysSinceLastHit": 0}

    r = srow.iloc[0]
    rank_pos = _safe_int(r.get("RankPos")) or 999999
    hits_per_week = float(r.get("HitsPerWeek", 0.0) or 0.0)
    hits = int(_safe_int(r.get("HitsWindow")) or 0)
    dslh_raw = r.get("DaysSinceLastHit")
    try:
        dslh = float(dslh_raw) if dslh_raw is not None and not pd.isna(dslh_raw) else 0.0
    except Exception:
        dslh = 0.0

    due_lo, due_hi = int(due_ranks[0]), int(due_ranks[1])

    if rank_pos <= int(top_n):
        bucket_label = "Top12"
        due_pressure = 0.0
    elif due_lo <= rank_pos <= due_hi:
        bucket_label = "Due"
        due_pressure = dslh
    else:
        bucket_label = "Other"
        # Keep a small "due" signal for outside-rank streams without overpowering BaseScore.
        due_pressure = dslh * 0.25

    # Optional: 24h presence boost (soft)
    if include_24h and df_24 is not None and not df_24.empty:
        try:
            core_key = canonical_core_key(str(core)) if core is not None else ""
            members = members_from_core(core_key, "AABC") if core_key else []
            if members:
                present = ((df_24.get("Stream") == stream) & (df_24.get("Box").isin(members))).any()
                if bool(present):
                    due_pressure += 5.0
        except Exception:
            pass
    ...
    seed_key = ""
    try:
        if seed_core_key == "core":
            seed_key = canonical_core_key(str(core)) if core is not None else ""
        else:
            seed_key = canonical_core_key(str(core)) if core is not None else ""
    except Exception:
        seed_key = str(core) if core is not None else ""

    return {
        "Top12": bucket_label,
        "BucketPick": bucket_label,
        "SeedKey": seed_key,
        "DuePressure": float(due_pressure),
        "HitsPerWeek": float(hits_per_week),
        "Hits": int(hits),
        "DaysSinceLastHit": int(_safe_int(dslh) or 0),
    }
...
def engine_cluster_positions(
    ...
    """Human-readable anchor cluster for the densest winner rank positions."""
    if not top_positions:
        return f"{label}: (no dense positions found)"
    ...
    if not cleaned:
        return f"{label}: (no numeric positions found)"

    cleaned = sorted(set(cleaned))
    cluster = cleaned[: max(1, int(cluster_size))]
    return f"{label}: {{{', '.join(str(i) for i in cluster)}}}"
...
with st.sidebar:
    ...
    # Precompute AABC members for each selected core (used by Northern Lights).
    CORE_MEMBERS = {}
    for _c in cores:
        try:
            CORE_MEMBERS[int(_c)] = members_from_core(str(_c), "AABC")
        except Exception:
            continue
    ...
    # Back-compat aliases for the Northern Lights master playlist (keep the rest of the app unchanged).
    if not hasattr(cfg, "top12"):
        cfg.top12 = cfg.top_base
    if not hasattr(cfg, "due_ranks"):
        cfg.due_ranks = (cfg.due_from_rank, cfg.due_to_rank)
    if not hasattr(cfg, "seed_core_key"):
        cfg.seed_core_key = "core"
    if not hasattr(cfg, "include_24h_signals"):
        cfg.include_24h_signals = True
    if not hasattr(cfg, "pos_strength_weight"):
        cfg.pos_strength_weight = 0.25
    if not hasattr(cfg, "max_master_rows"):
        cfg.max_master_rows = 200
    if not hasattr(cfg, "max_final_rows"):
        cfg.max_final_rows = 200
...
with tabs[0]:
    ...
    run_now = st.button("Run / refresh Northern Lights now")
    nl_autorun = st.checkbox("Auto-run when inputs change", value=False)
