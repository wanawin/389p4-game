...
if (not out.empty):
    ...
    with st.expander("Open Auto Profit Planner", expanded=False):
        ...
        with ap6:
            # Default: the rows that correspond to your "Top12 BaseScore" concept
            default_rows = list(range(1, 13))
        ...
        with ap7:
            member_mode_ap = st.selectbox(
                "Box ticket member mode",
                [
                    "BOX: Play Top1 member pick (recommended if strong)",
                    "BOX: Play Top2 member picks",
                    "BOX: Play all 3 family members",
                ],
                index=0,
                key="ap_member_mode",
            )
        ...
        if ev.empty:
            ...
            # Define BOX win per row under the chosen member mode
