...
def top_dense_positions(pos_map: pd.DataFrame, top_n: int = 10) -> List[int]:
    ...
    rankpos = tmp["RankPos"]
    if isinstance(rankpos, pd.DataFrame):
        rankpos = rankpos.iloc[:, 0]
    tmp["RankPos_num"] = pd.to_numeric(rankpos, errors="coerce")
...
def engine_cluster_positions(
    ...
    - If passed a list of positions, formats that list.
    - If passed a df of hits + base_stats, builds a position map and derives top-N positions.
    """
    # Case 1: already a list/tuple/set of positions
    if isinstance(top_positions_or_hits, (list, tuple, set)):
        top_positions = list(top_positions_or_hits)
    # Case 2: passed df_hits (+ optional base_stats) — derive positions first
    elif isinstance(top_positions_or_hits, pd.DataFrame):
        df_hits = top_positions_or_hits
        if base_stats is None or not isinstance(base_stats, pd.DataFrame) or df_hits is None or df_hits.empty:
            return f"{label}: (no dense positions found)"

        bs = base_stats.copy()
        if "RankPos" not in bs.columns and "RankPos_full" in bs.columns:
            bs["RankPos"] = bs["RankPos_full"]

        # Need Stream + RankPos to compute dense positions
        if "Stream" not in df_hits.columns or "Stream" not in bs.columns or "RankPos" not in bs.columns:
            return f"{label}: (no dense positions found)"

        hit_counts = df_hits.groupby("Stream").size().rename("HitCount").reset_index()
        pos_map = hit_counts.merge(bs[["Stream", "RankPos"]], on="Stream", how="left").dropna(subset=["RankPos"])
...
def evaluate_rare_engine(
    ...
    if _vals is None:
        _vals = []
    elif isinstance(_vals, str) or (not is_list_like(_vals)):
        _vals = [_vals]
    out["R4_24h_cluster_top10pos"] = out["RankPos_full"].isin(list(_vals)) if enable_r4 else False
...
def evaluate_ultra_rare_engine(
    ...
    if _vals is None:
        ...
        _vals = [_vals]
    out["Q4_24h_cluster_top5pos"] = out["RankPos_full"].isin(list(_vals)) if enable_q4 else False
...
with tabs[0]:
    ...
    # Controls for Northern Lights computation
    run_now = st.button("Run / refresh Northern Lights now", key="nl_run_now")
    nl_autorun = st.checkbox("Auto-run when inputs change", value=False, key="nl_autorun")
