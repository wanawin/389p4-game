...
APP_VERSION = "v52 (Fix: position percentile map unpack compat)"

CHANGE_LOG_V51 = """v51 — SeedTraits + Cadence + AllCores Cache-Only (built from v50, NO regressions
v52 — Fix: position percentile map unpack compat (prevents ValueError: too many values to unpack))
...
def build_allcores_rankpos_pctmap(
    ...
    _res_ppm = position_percentile_map(comb)
    pct = _res_ppm[0] if isinstance(_res_ppm, (tuple, list)) else _res_ppm
    return pct
...
# -------------------------
# Seed Traits (positive/negative) + Cadence (v52)
# -------------------------
...
def get_position_percentiles_cached(core: str, window_days: int, stream_stats: pd.DataFrame) -> pd.DataFrame:
    ...
    _res_ppm = position_percentile_map(stream_stats)


    # Backward/forward compatible: position_percentile_map may return a DF or (DF, meta)


    pos_map = _res_ppm[0] if isinstance(_res_ppm, (tuple, list)) else _res_ppm
    cache[key] = pos_map
...
with st.sidebar:
    ...
    st.divider()
    st.subheader("Seed Traits + Cadence (v52)")
...
# ---- v52: Seed Traits + Cadence data (autoload + optional uploads)
...
def get_pos_map_cached(core: str, window_days: int, stream_stats_df, last_date: str | None):
    ...
    _res_ppm = position_percentile_map(stream_stats_df)
    pos_df = _res_ppm[0] if isinstance(_res_ppm, (tuple, list)) else _res_ppm
        ...
        if not rows:
            ...
            with st.expander("Northern Star percentile map (playlist positions)", expanded=False):
                try:
                    ...
                    _res_ppm = position_percentile_map(_best[["RankPos", "HitsWindow"]].copy())
                    _pos = _res_ppm[0] if isinstance(_res_ppm, (tuple, list)) else _res_ppm
