...
# pk4_northern_star_app_2026-02-04_v33.py
...
with st.sidebar:
    ...
    # Available cores (derived from the uploaded history so the UI never crashes)
    def _extract_available_cores(_df: pd.DataFrame) -> list[str]:
        if _df is None or _df.empty:
            return []
        col = None
        for c in ("Core", "core", "CORE"):
            if c in _df.columns:
                col = c
                break
        if col is None:
            return []
        vals = _df[col].dropna().tolist()
        out = []
        for v in vals:
            s = str(v).strip()
            if not s or s.lower() == "nan":
                continue
            # Normalize floats like 19.0 -> 19
            try:
    ...
    available_cores = ["389"]  # fallback until df is loaded
    if master_file is not None:
        try:
            _ck = f"__available_cores__{getattr(master_file, 'name', 'uploaded')}__{len(master_file.getvalue())}"
        except Exception:
            _ck = "__available_cores__uploaded__0"
        if _ck in st.session_state:
            available_cores = st.session_state[_ck]
st.header("Core selection")

# Build available core list from data + any built-in preset list if present
available_cores = []
try:
    # If a preset dict exists, include its keys (e.g., {"389": {...}, ...})
    available_cores = [str(k).zfill(3) for k in list(CORE_PRESETS.keys())]  # type: ignore[name-defined]
except Exception:
    available_cores = []

# Include any cores found in the uploaded history data
try:
    for c in sorted(list(all_cores_in_data)):
        cc = str(c).zfill(3)
        if cc not in available_cores:
            available_cores.append(cc)
except Exception:
    pass
...
# Determine a stable default core (never reference cfg here; cfg is created later)
try:
    default_core = str(getattr(RankConfig(), "default_core", "389")).zfill(3)
except Exception:
    default_core = "389"

if default_core not in available_cores:
    available_cores = [default_core] + [c for c in available_cores if c != default_core]

available_cores = sorted(list(dict.fromkeys([str(c).zfill(3) for c in available_cores])))

# --- View core (single-core tab) ---
if "view_core" not in st.session_state:
    st.session_state["view_core"] = default_core

_view_core = str(st.session_state.get("view_core", default_core)).zfill(3)
view_core = st.selectbox(
    "View core (dropdown)",
    options=available_cores,
    index=available_cores.index(_view_core) if _view_core in available_cores else 0,
    key="view_core",
)

# --- Multi-core cache/batch selection ---
# Initialize once; do NOT reset on every rerun (prevents 'default disappears' behavior)
if "cores_for_cache" not in st.session_state or not isinstance(st.session_state["cores_for_cache"], list):
    st.session_state["cores_for_cache"] = [view_core]
...
# Always keep the currently-viewed core included (but don't wipe other selections)
if view_core not in cores_for_cache:
    cores_for_cache = [view_core] + list(cores_for_cache)
    st.session_state["cores_for_cache"] = cores_for_cache

show_tabs_all_cores = st.checkbox(
    "Show tabs for all selected cores (can be slower)",
    value=bool(st.session_state.get("show_tabs_all_cores", False)),
    key="show_tabs_all_cores",
)
# Which cores should downstream modules compute?
cores = list(st.session_state.get("cores_for_cache", [str(st.session_state.get("view_core", "389")).zfill(3)]))
show_tabs = bool(st.session_state.get("show_tabs_all_cores", False))


# Which cores should downstream modules compute?
cores = st.session_state["cores_for_cache"]  # always compute selected cores
