...
def predict_core_member_seeded(
    df_all: pd.DataFrame,
    core_key: str,
    test_date: pd.Timestamp,
    window_days: int,
    *,
    ...
    include_rare: bool = False,
    seed: str | None = None,
    enable_member_seed_traits: bool = True,
    member_pos_lookup: Dict[int, Dict[str, Dict[str, Dict[str, float]]]] | None = None,
    member_neg_lookup: Dict[int, Dict[str, Dict[str, Dict[str, float]]]] | None = None,
) -> dict[str, Any]:
    """predict_core_member + optional seed-trait member override (walk-forward safe)."""
    mp = predict_core_member(
        df_all,
        core_key,
        test_date,
        window_days,
    ...
    if not enable_member_seed_traits:
        return mp
    base_rank = [x for x in [mp.get("top1"), mp.get("top2")] if x]
    if not base_rank:
        return mp
    ranked = apply_member_trait_overrides(
        core_key,
        seed,
        base_rank,
        pos_lookup=member_pos_lookup,
        neg_lookup=member_neg_lookup,
    )
    if ranked and ranked != base_rank:
        mp["top1"] = ranked[0] if len(ranked) >= 1 else mp.get("top1")
        mp["top2"] = ranked[1] if len(ranked) >= 2 else mp.get("top2")
    return mp
...
def _core_digits_from_key(core_key: str) -> list[str]:
    s = str(core_key).zfill(3)
    return sorted(list(s))

def _label_for_repeat_digit(core_key: str, repeat_digit: str) -> str | None:
    digs = _core_digits_from_key(core_key)
    if repeat_digit not in digs:
        return None
    # AABC repeats smallest digit; ABBC repeats middle; ABCC repeats largest
    if repeat_digit == digs[0]:
        return "AABC"
    if repeat_digit == digs[1]:
        return "ABBC"
    if repeat_digit == digs[2]:
        return "ABCC"
    return None

def _build_member_traits_lookup(df: pd.DataFrame) -> Dict[int, Dict[str, Dict[str, Dict[str, float]]]]:
    """lookup[core_family][trait][value][label] -> lift (label in {AABC,ABBC,ABCC})."""
    lookup: Dict[int, Dict[str, Dict[str, Dict[str, float]]]] = {}
    if df is None or df.empty:
        return lookup
    ...
    member_col = cols.get("member", None)  # canonical 4-digit member like 1169
    rep_col = cols.get("repeat_digit", None)
    ...
    for _, r in df.iterrows():
        try:
            core_fam = int(r[core_col])
        except Exception:
            continue
        trait = str(r[trait_col])
        val = str(r[val_col])

        repeat_digit = None
        if rep_col and pd.notna(r.get(rep_col, None)):
            repeat_digit = str(int(r[rep_col]))
        if (repeat_digit is None or repeat_digit == "nan") and member_col and pd.notna(r.get(member_col, None)):
            mstr = str(r[member_col]).zfill(4)
            repeat_digit = mstr[0]

        if repeat_digit is None:
            continue

        label = _label_for_repeat_digit(str(core_fam), str(repeat_digit))
        if label is None:
            continue
        ...
        lookup.setdefault(core_fam, {}).setdefault(trait, {}).setdefault(val, {})[label] = lift

    return lookup

def apply_member_trait_overrides(
    core_key: str,
    seed: str | None,
    base_rank: list[str],
    *,
    pos_lookup: Dict[int, Dict[str, Dict[str, Dict[str, float]]]] | None = None,
    neg_lookup: Dict[int, Dict[str, Dict[str, Dict[str, float]]]] | None = None,
) -> list[str]:
    """Re-rank member labels (AABC/ABBC/ABCC) using seed-trait member lifts as a soft override.

    - base_rank is the MODE/LAST order (top1, top2, ...)
    - if no seed or no lookups, returns base_rank unchanged
    """
    if not seed or not base_rank:
        return base_rank
    try:
        core_fam = int(str(core_key))
    except Exception:
        return base_rank
    pos_lookup = pos_lookup or {}
    neg_lookup = neg_lookup or {}

    # compute same seed feature dict used by core seed traits scorer
    feats = compute_seed_feature_values(seed, core_key=core_key, stream=None, last5_union_digits_by_stream=None)
    # feats is Dict[str,str]; we only need those keys that exist in lookup
    scores = {lab: 1.0 for lab in base_rank}
    for trait, val in (feats or {}).items():
        try:
            v = str(val)
        except Exception:
            continue
        if core_fam in pos_lookup and trait in pos_lookup[core_fam] and v in pos_lookup[core_fam][trait]:
            for lab, lift in pos_lookup[core_fam][trait][v].items():
                if lab in scores and isinstance(lift, (int, float)) and lift > 0:
                    scores[lab] *= float(lift)
        if core_fam in neg_lookup and trait in neg_lookup[core_fam] and v in neg_lookup[core_fam][trait]:
            for lab, lift in neg_lookup[core_fam][trait][v].items():
                if lab in scores and isinstance(lift, (int, float)) and lift > 0:
                    # negative file lift < 1.0: multiply down
                    scores[lab] *= float(lift)

    # stable sort: higher score first, tie keep base order
    ranked = sorted(base_rank, key=lambda lab: (-scores.get(lab, 1.0), base_rank.index(lab)))
    return ranked
    # normalize columns
    cols = {c.lower(): c for c in df.columns}
    core_col = cols.get("core_family", None)
...
with st.sidebar:
    ...
    traits_pos_extra_file = st.file_uploader("Seed traits EXTRA POS (optional)", type=["csv"], key="traits_pos_extra_file")
    traits_neg_extra_file = st.file_uploader("Seed traits EXTRA NEG (optional)", type=["csv"], key="traits_neg_extra_file")
    member_traits_pos_file = st.file_uploader("Member traits POS (optional)", type=["csv"], key="member_traits_pos_file")
    member_traits_neg_file = st.file_uploader("Member traits NEG (optional)", type=["csv"], key="member_traits_neg_file")

    enable_seed_traits = st.checkbox("Enable Seed Traits boost (soft only)", value=True, key="enable_seed_traits")
    enable_member_seed_traits = st.checkbox("Enable Member Seed-Trait overrides (soft, after MODE/LAST)", value=True, key="enable_member_seed_traits")
...
# v58+: Extra seed/core traits + member-level traits (generated from history; optional but recommended)
...
seed_traits_pos_base_df = _read_local_or_uploaded_csv(globals().get("traits_pos_file", None), DEFAULT_TRAITS_POS_PATH)
seed_traits_neg_base_df = _read_local_or_uploaded_csv(globals().get("traits_neg_file", None), DEFAULT_TRAITS_NEG_PATH)
seed_traits_pos_extra_df = _read_local_or_uploaded_csv(globals().get("traits_pos_extra_file", None), DEFAULT_TRAITS_POS_EXTRA_PATH)
seed_traits_neg_extra_df = _read_local_or_uploaded_csv(globals().get("traits_neg_extra_file", None), DEFAULT_TRAITS_NEG_EXTRA_PATH)
seed_traits_pos_df = pd.concat([d for d in [seed_traits_pos_base_df, seed_traits_pos_extra_df] if d is not None and not d.empty], ignore_index=True) if ((seed_traits_pos_base_df is not None and not seed_traits_pos_base_df.empty) or (seed_traits_pos_extra_df is not None and not seed_traits_pos_extra_df.empty)) else pd.DataFrame()
seed_traits_neg_df = pd.concat([d for d in [seed_traits_neg_base_df, seed_traits_neg_extra_df] if d is not None and not d.empty], ignore_index=True) if ((seed_traits_neg_base_df is not None and not seed_traits_neg_base_df.empty) or (seed_traits_neg_extra_df is not None and not seed_traits_neg_extra_df.empty)) else pd.DataFrame()
...
# Member-level trait tables (optional). Used as a soft override AFTER MODE/LAST.
...
member_traits_pos_lookup = _build_member_traits_lookup(member_traits_pos_df)
member_traits_neg_lookup = _build_member_traits_lookup(member_traits_neg_df)
            ...
            for core in hit_cores:
                ...
                if member_track:
                    ...
                    if mk not in member_pred_cache:
                        ...
                        member_pred_cache[mk] = predict_core_member_seeded(
                            df_all,
                            core,
                            pd.to_datetime(test_date).normalize(),
                            ...
                            enable_member_seed_traits=st.session_state.get("enable_member_seed_traits", True),
                            member_pos_lookup=member_traits_pos_lookup,
                            member_neg_lookup=member_traits_neg_lookup,
                        )

                    # (legacy call replaced)
                    # member_pred_cache[mk] = predict_core_member(...)  # replaced by predict_core_member_seeded
