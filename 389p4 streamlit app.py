...
def build_12_straights_table(ordering_stats: pd.DataFrame, family_choice: str | int | None = None) -> pd.DataFrame:
    # Build a stable 12-row table (all unique permutations for the selected family), even if some have zero counts.
    # ordering_stats is expected to have columns: Straight, Count, Pct (from compute_straight_ordering_stats).
    fam = str(family_choice) if family_choice is not None else None
    if fam is None:
        # Try to infer family from the first straight; fall back to 3389.
        if isinstance(ordering_stats, pd.DataFrame) and not ordering_stats.empty and 'Straight' in ordering_stats.columns:
            s0 = str(ordering_stats['Straight'].iloc[0])
            fam = ''.join(sorted(s0))
        else:
            fam = '3389'
    if fam not in {'3389','3889','3899'}:
        # Defensive: if user passed something unexpected, keep app alive.
        fam = '3389'

    from itertools import permutations
    digits = list(fam)
    all_straights = sorted({''.join(p) for p in permutations(digits, 4)})

    # Normalize incoming stats
    stats = ordering_stats.copy() if isinstance(ordering_stats, pd.DataFrame) else pd.DataFrame(columns=['Straight','Count','Pct'])
    ...
    merged = merged.sort_values(['Count','Straight'], ascending=[False, True]).reset_index(drop=True)
    merged.insert(0, 'Rank', merged.index + 1)
    # Keep a consistent column order for display/download
    return merged[['Rank','Straight','Count','Pct']]
