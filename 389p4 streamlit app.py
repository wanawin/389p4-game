...
if last_all is not None and not pd.isna(last_all):
    st.sidebar.write(f"**All‑states history most recent date:** {pd.to_datetime(last_all).date()}  (age: {_age_days(last_all)} days)")
else:
    st.sidebar.write("**All‑states history most recent date:** (not found)")

if map24_file is None:
    st.sidebar.write("**24h map file:** (not uploaded)")
elif last_24 is not None and not pd.isna(last_24):
    st.sidebar.write(f"**24h map most recent date:** {pd.to_datetime(last_24).date()}  (age: {_age_days(last_24)} days)")
else:
    st.sidebar.write("**24h map most recent date:** (not found)")
