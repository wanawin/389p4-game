...
with c2:
    ...
    if df_master_show is None:
        ...
        with st.expander("Northern Lights: Member-level scored plays (display only)", expanded=False):
            df_plays_show = st.session_state.get("_nl_plays")
            if df_plays_show is None:
                st.info("Run Northern Lights first to generate member-level plays.")
            elif df_plays_show.empty:
                st.warning("No member-level plays were generated for the current Northern Lights run.")
            else:
                # Filters (display only)
                fcols = st.columns([1, 2, 2, 1])
                with fcols[0]:
                    core_filter = st.selectbox(
                        "Filter core (optional)",
                        options=["(All)"] + sorted({str(int(x)).zfill(3) for x in df_plays_show["Core"].dropna().unique()}),
                        index=0,
                        key="nl_member_core_filter",
                    )
                with fcols[1]:
                    stream_opts = sorted({str(x) for x in df_plays_show["Stream"].dropna().unique()})
                    stream_filter = st.multiselect(
                        "Filter stream(s) (optional)",
                        options=stream_opts,
                        default=[],
                        key="nl_member_stream_filter",
                    )
                with fcols[2]:
                    show_cols = st.multiselect(
                        "Columns",
                        options=[c for c in df_plays_show.columns],
                        default=[c for c in df_plays_show.columns if c not in ("SeedKey",)],
                        key="nl_member_cols",
                    )
                with fcols[3]:
                    top_n = st.number_input("Rows", min_value=10, max_value=5000, value=250, step=10, key="nl_member_topn")

                df_disp = df_plays_show.copy()
                if core_filter != "(All)":
                    try:
                        df_disp = df_disp[df_disp["Core"].astype(int).astype(str).str.zfill(3) == core_filter]
                    except Exception:
                        pass
                if stream_filter:
                    df_disp = df_disp[df_disp["Stream"].astype(str).isin([str(x) for x in stream_filter])]

                df_disp = df_disp.head(int(top_n))

                st.dataframe(df_disp[show_cols] if show_cols else df_disp, use_container_width=True, hide_index=True)

                # Download (display only)
                try:
                    csv_bytes = df_disp.to_csv(index=False).encode("utf-8")
                    st.download_button(
                        "Download member-level plays (CSV)",
                        data=csv_bytes,
                        file_name="northern_lights_member_level_plays.csv",
                        mime="text/csv",
                        key="dl_nl_member_level_csv",
                    )
                except Exception:
                    st.caption("Download unavailable for this view.")
