...
with st.sidebar:
    ...
    available_cores = [str(getattr(cfg, "default_core", "389")).zfill(3)]
    if master_file is not None:
        ...
        if _ck in st.session_state:
            ...
            try:
                _tmp = try_read_tablelike(master_file)
                _cores = _extract_available_cores(_tmp)
                if _cores:
                    available_cores = _cores
            except Exception:
                pass
            st.session_state[_ck] = available_cores
st.header("Core selection")

# ---- View (single core) vs Batch (multi-core) selections ----
view_core = st.selectbox(
    "View core (dropdown)",
    options=available_cores,
    index=available_cores.index(str(cfg.core_default)) if str(cfg.core_default) in available_cores else 0,
    key="view_core",
)

# Persist multi-core selection across reruns, and always include the current view_core
if "cores_for_cache" not in st.session_state or not isinstance(st.session_state.get("cores_for_cache"), list):
    st.session_state["cores_for_cache"] = [view_core]

cores_for_cache = st.multiselect(
    "Cores to include for cache building / batch tools",
    options=available_cores,
    default=st.session_state.get("cores_for_cache", [view_core]),
    key="cores_for_cache",
)

# Ensure view_core never disappears from batch selection (this was the root of the "no multi-core" behavior)
if view_core not in cores_for_cache:
    cores_for_cache = [view_core] + list(cores_for_cache)
    # de-dupe while preserving order
    seen = set()
    cores_for_cache = [c for c in cores_for_cache if not (c in seen or seen.add(c))]
    st.session_state["cores_for_cache"] = cores_for_cache

show_tabs = st.checkbox("Show tabs for all selected cores (can be slower)", value=False, key="show_tabs")
