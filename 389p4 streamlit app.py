...
APP_VERSION = "v51 (SeedTraits+Cadence+AllCoresCacheOnly)"
...
"""
# ------------------------------------------------------------
# MASTER CHECKLISTS (embedded in-app to prevent omissions)
# ------------------------------------------------------------

MASTER_CHECKLIST_MD = r"""
# Master checklist (so nothing gets omitted again)

## A) Core & Core View (you said 1–2 are done)

- ✅ Multi-core selection (cache building + batch tools)
- ✅ Show tabs for all selected cores (optional) under Core view
- ✅ Core ranking percentile map (tie-breaker) display (Core view)

## B) Northern Star View (currently the trouble spot)

- ⬜ Northern Star percentile map shown on Northern Star page (not just one ranked list)
- ⬜ Northern Star “extra panel(s)” that existed before (you said something else used to be there)

## C) “Northern Lights” master playlist (cross-core)

- ⬜ Northern Lights master list (all streams, best→worst), using cached baseline + today climate

## D) Self-maintenance (updating / purging / cache hygiene)

- ⬜ Cache Builder + Rebuild controls (baseline math vs daily climate math split)
- ⬜ Cache purge / versioning / “as-of date” freshness UI
- ⬜ Safe append/update workflow (if your design includes it) without recomputing everything daily

## E) Cadence + core-specific “behavior” signals (per-core differences)

- ⬜ Cadence boost as scoring (never hard-eliminate) tied to your cadence report
- ⬜ Per-core trait lifts (sum/spread/root/parity/HL/etc.) integrated as weights (from files + historical computation)

## F) Trigger Map (prev-draw feature weighting)

- ⬜ Trigger Map panel + weighting (your “7890” override + last-digit decision tree) integrated into “Play Today” scoring

## G) Straights module

- ⬜ Straights generation / ranking (as optional last step; box always included)
"""

V51_INTEGRATION_PASS_MD = r"""
# ✅ What would be added in v51 (the correct, non-breaking plan)

v51 should include ALL of this in one integration pass:

## ✅ (1) Seed Traits (POS + NEG) integrated (real data only)

- autoload trait CSVs if present  
- otherwise uploader appears  
- scoring:
  - positive lift multiplies score  
  - negative lift reduces score  
  - clamp to prevent runaway overfitting  
- visible explanation expander: “why this stream moved up/down”

## ✅ (2) Cadence integrated (180 default, 365 toggle)

- cadence computed per core per stream using last-hit gaps (real history)  
- added as score component (soft)  
- cadence shown in table (band: FRESH/WARM/DUE/OVERDUE)  
- does not eliminate anything

## ✅ (3) “Select All Cores” + “Clear” buttons

- works in multi-core mode  
- doesn’t freeze UI  
- doesn’t change core presets list

## ✅ (4) Percentile maps panel (3 layers, not redundant)

- Global / All cores map  
- Per-core map  
- Northern Star map (global lens)  
- …and show all 3 cleanly with labels and update buttons.

## ✅ (5) Master checklist embedded in app

- MASTER_CHECKLIST.md rendered inside Streamlit  
- also shown in sidebar status (short summary)

## ✅ (6) System Check tab (regression proof)

- verifies every required feature is present every run  
- prints “PASS ✅ / FAIL ❌”  
- makes it impossible to deploy a broken version without noticing
"""
...
def build_allcores_rankpos_pctmap(
    cores: List[str],
    window_days: int,
    expected_last_date: Optional[str] = None,
    cache_only: bool = True,
) -> Tuple[Optional[pd.DataFrame], List[str]]:
    """Build a *global* rank-position percentile map aggregated across many cores.

    This is used as a tie-breaker when two streams are otherwise close.

    Returns:
        (pct_map_df_or_None, missing_cores)

    NOTE:
      - We intentionally rely on on-disk per-core baseline caches when cache_only=True.
      - Missing or stale baselines should NOT crash the app.
    """
    ...
    for core in cores:
        if cache_only:
            ss, _pos, meta = _load_baseline_from_disk(
                core, window_days, expected_last_date=expected_last_date
            )
            if ss is None or ss.empty:
                missing.append(core)
                continue
            # If caller supplied an expected last date, enforce it (prevents accidental lookahead).
            last_date = (meta or {}).get("last_date")
            if expected_last_date and last_date and last_date != expected_last_date:
                missing.append(core)
                continue
        else:
            # Non-cache path intentionally not implemented here because it would require
            # passing full history; keeping behaviour deterministic.
            missing.append(core)
            continue
    ...
    pct = position_percentile_map(comb)
    return pct, missing
...
def render_straights_shortlist(nl_df: pd.DataFrame) -> None:
    """Generate a straight-play shortlist from the Northern Lights playlist.

    - Does NOT change rankings.
    - Designed to be safe/fast: user controls top-N rows and max output size.
    """
    if nl_df is None or nl_df.empty:
        st.info("No playlist available yet. Build Northern Lights first.")
        return

    st.caption("Straights are optional and are generated **after** the box-based playlist. This shortlist is derived from the top rows of Northern Lights.")

    # Controls to keep it safe
    top_n = st.slider("Use top N playlist rows", min_value=5, max_value=min(250, int(len(nl_df))), value=min(30, int(len(nl_df))), step=5)
    max_straights = st.slider("Max straight plays to output", min_value=50, max_value=2000, value=300, step=50)
    include_members = st.checkbox("Include all 3 family members per core (AABC/ABBC/ABCC)", value=True)

    # Identify key columns (be defensive)
    col_core = "Core" if "Core" in nl_df.columns else None
    col_stream = "Stream" if "Stream" in nl_df.columns else None

    rows = nl_df.head(top_n).copy()
    out = {}

    for idx, r in rows.iterrows():
        core = str(r.get(col_core, "")).zfill(3) if col_core else ""
        stream = str(r.get(col_stream, "")) if col_stream else ""
        if not core or not core.isdigit():
            continue

        members = members_from_core(core)
        if not include_members and members:
            members = members[:1]

        for m in members:
            for s in unique_straights_for_box(m):
                # Keep the best (lowest) source rank/index
                if s not in out or idx < out[s]["SourceIdx"]:
                    out[s] = {
                        "Straight": s,
                        "FromMember": m,
                        "Core": core,
    ...
    if not out:
        st.info("No straights generated from the selected playlist rows.")
        return

    df_out = pd.DataFrame(list(out.values())).sort_values(["SourceIdx", "Core", "Stream", "Straight"], ascending=[True, True, True, True])
    df_out = df_out.head(max_straights).reset_index(drop=True)

    st.subheader("Straights shortlist")
    st.dataframe(df_out, use_container_width=True, height=520)

    # Copy/paste-friendly
    st.caption("Copy/paste (one per line):")
    st.text_area("Straights (copy/paste)", value="\n".join(df_out["Straight"].astype(str).tolist()), height=220)

    # Download
    st.download_button(
        "Download straights shortlist (CSV)",
        data=df_out.to_csv(index=False).encode("utf-8"),
        file_name="straights_shortlist.csv",
        mime="text/csv",
    )


def _backtest_fast_scorecard(df_all: pd.DataFrame, cores: list[str], window_days: int, streams_limit: list[str] | None = None) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Fast scorecard backtest (static ranking computed once per core)."""
    df = df_all.copy()
    if streams_limit:
        df = df[df["Stream"].isin(streams_limit)].copy()
    if df.empty:
    ...
    for core in [str(c).zfill(3) for c in cores]:
        # Stats for ranking (computed on the same window to match what the app does)
        stats = compute_stream_stats(df_win, core, window_days=window_days, exclude_md=False)
        if stats is None or stats.empty:
            continue

        rank_by_stream = dict(zip(stats["Stream"].astype(str), stats["RankPos"].astype(int)))

        hits = compute_core_hits(df_win, core, structures=("AABC",))
        if hits is None or hits.empty:
            summaries.append({
                "Core": core, "HitEvents": 0,
                "Top1%": 0.0, "Top3%": 0.0, "Top5%": 0.0, "Top10%": 0.0,
                "MedianRank": np.nan, "MeanRank": np.nan
            })
            continue

        # Map rank positions for hit events
        hits = hits.copy()
        hits["RankPos"] = hits["Stream"].astype(str).map(rank_by_stream)
        hits = hits.dropna(subset=["RankPos"]).copy()
        hits["RankPos"] = hits["RankPos"].astype(int)
        ...
        n = len(hits)
        top1 = float((hits["RankPos"] <= 1).sum()) / n * 100.0
        top3 = float((hits["RankPos"] <= 3).sum()) / n * 100.0
        top5 = float((hits["RankPos"] <= 5).sum()) / n * 100.0
        top10 = float((hits["RankPos"] <= 10).sum()) / n * 100.0
        ...
        summaries.append({
            ...
            "Top5%": round(top5, 2),
            "Top10%": round(top10, 2),
            "MedianRank": float(np.median(hits["RankPos"])),
            "MeanRank": float(np.mean(hits["RankPos"])),
        })

        # Events for distribution chart/table
        hits["Core"] = core
        all_events.append(hits[["Date","Stream","Result","Core","RankPos"]])

    sum_df = pd.DataFrame(summaries).sort_values(["Top10%","Top5%","Top3%","Top1%","HitEvents"], ascending=[False, False, False, False, False])
    events_df = pd.concat(all_events, ignore_index=True) if all_events else pd.DataFrame()
    return sum_df, events_df


def _backtest_deep_walkforward(df_all: pd.DataFrame, cores: list[str], window_days: int, test_days: int, max_transitions: int, streams_limit: list[str] | None = None) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Deep walk-forward (reranks per day using trailing window). Capped to avoid hangs."""
    df = df_all.copy()
    if streams_limit:
        df = df[df["Stream"].isin(streams_limit)].copy()
    if df.empty:
    ...
    df = df.sort_values(["Date","Stream"]).reset_index(drop=True)
    end_date = df["Date"].max()
    start_date = end_date - pd.Timedelta(days=int(test_days) - 1)
    dates = sorted(df[df["Date"] >= start_date]["Date"].unique())

    cores = [str(c).zfill(3) for c in cores]
    # Precompute core box sets for fast hit checks
    core_boxes = {c: set(members_from_core(c)) for c in cores}
    ...
    for core in cores:
        ...
        for d in dates:
            ...
            # Training data up to day-1, trimmed to trailing window_days
            train = df[df["Date"] < d].copy()
            if train.empty:
                continue
            train_end = train["Date"].max()
            train_start = train_end - pd.Timedelta(days=int(window_days) - 1)
            train = train[train["Date"] >= train_start].copy()

            stats = compute_stream_stats(train, core, window_days=window_days, exclude_md=False)
            if stats is None or stats.empty:
                transitions += 1
                continue
            ...
            # Evaluate actual day d results for this core
            day_df = df[df["Date"] == d].copy()
            if day_df.empty:
                transitions += 1
                continue

            boxes = core_boxes.get(core, set())
            day_hits = day_df[day_df["Box"].astype(str).isin(boxes)].copy() if "Box" in day_df.columns else day_df[day_df["Result"].astype(str).isin(boxes)].copy()
            if not day_hits.empty:
                day_hits["RankPos"] = day_hits["Stream"].astype(str).map(rank_by_stream)
                day_hits = day_hits.dropna(subset=["RankPos"]).copy()
                day_hits["RankPos"] = day_hits["RankPos"].astype(int)
                if not day_hits.empty:
                    for _, r in day_hits.iterrows():
                        hit_positions.append(int(r["RankPos"]))
                    day_hits["Core"] = core
                    core_events.append(day_hits[["Date","Stream","Result","Core","RankPos"]])

            transitions += 1

        if hit_positions:
            n = len(hit_positions)
            arr = np.array(hit_positions, dtype=int)
            summaries.append({
                "Core": core,
                "HitEvents": int(n),
                "Top1%": round(float((arr <= 1).sum())/n*100.0, 2),
                "Top3%": round(float((arr <= 3).sum())/n*100.0, 2),
                "Top5%": round(float((arr <= 5).sum())/n*100.0, 2),
                "Top10%": round(float((arr <= 10).sum())/n*100.0, 2),
        ...
        if core_events:
            all_events.append(pd.concat(core_events, ignore_index=True))
...
def render_backtest(df_all: pd.DataFrame, cores: list[str], window_days: int) -> None:
    """Backtest tab: Fast scorecard + optional deep walk-forward (capped)."""
    st.markdown("### What the Backtest does (real + won't crash)")
    st.markdown(
        """To keep it accurate **and** usable, Backtest has two modes:

1) **Fast Scorecard (default)**  
• Uses your real history file and shows: Top‑1/3/5/10 capture %, rank‑position distribution, per‑core + overall summary  
• **No per‑draw re‑ranking loops** (prevents hanging)

2) **Deep Walk‑Forward (optional, slower)**  
• Only runs if you click **Run deep backtest**  
• Can be limited by last N days, streams, and a max‑transitions cap
"""
    )
    ...
    cores = [str(c).zfill(3) for c in cores if str(c).strip()]
    if not cores:
        st.info("Select at least one core.")
        return

    # Optional stream limit
    all_streams = sorted(df_all["Stream"].astype(str).unique()) if "Stream" in df_all.columns else []
    streams_limit = st.multiselect("Limit to these streams (optional)", options=all_streams, default=[])

    mode = st.radio("Mode", ["Fast Scorecard (default)", "Deep Walk‑Forward (optional)"], index=0, horizontal=True)

    if mode.startswith("Fast"):
        sum_df, events_df = _backtest_fast_scorecard(df_all, cores, window_days, streams_limit or None)

        if sum_df is None or sum_df.empty:
            st.info("No backtest results for the current selection.")
            return

        st.subheader("Per-core scorecard")
        st.dataframe(sum_df, use_container_width=True, height=420)

        # Overall summary
        if events_df is not None and not events_df.empty:
            st.subheader("Overall scorecard (all selected cores)")
            n = len(events_df)
            rp = events_df["RankPos"].astype(int)
            overall = {
                "HitEvents": int(n),
                "Top1%": round(float((rp <= 1).sum())/n*100.0, 2),
                "Top3%": round(float((rp <= 3).sum())/n*100.0, 2),
                "Top5%": round(float((rp <= 5).sum())/n*100.0, 2),
                "Top10%": round(float((rp <= 10).sum())/n*100.0, 2),
                "MedianRank": float(np.median(rp)),
                "MeanRank": float(np.mean(rp)),
            }
            st.write(overall)

            st.subheader("Rank-position distribution (where winners fall)")
            dist = rp.value_counts().sort_index()
            st.dataframe(dist.rename_axis("RankPos").reset_index(name="HitEvents"), use_container_width=True, height=420)

            with st.expander("Hit events (debug)"):
                st.dataframe(events_df.sort_values(["Date","Core","RankPos"]), use_container_width=True, height=520)

    else:
        st.warning("Deep Walk‑Forward is more accurate but slower. Keep caps small to avoid hanging.")
        test_days = st.slider("Test last N days", min_value=14, max_value=365, value=60, step=7)
        max_transitions = st.slider("Max transitions cap (safety)", min_value=20, max_value=2000, value=200, step=20)

        run = st.button("Run deep backtest", type="primary")
        if not run:
            st.info("Click **Run deep backtest** to start.")
            return

        prog = st.progress(0.0)
        status = st.empty()

        # Run (single pass; progress is coarse)
        sum_df, events_df = _backtest_deep_walkforward(df_all, cores, window_days, test_days, max_transitions, streams_limit or None)
        prog.progress(1.0)
        status.success("Deep backtest complete.")

        if sum_df is None or sum_df.empty:
            st.info("No deep backtest results for the current selection.")
            return

        st.subheader("Per-core walk-forward scorecard")
        st.dataframe(sum_df, use_container_width=True, height=420)

        if events_df is not None and not events_df.empty:
            st.subheader("Overall walk-forward scorecard (all selected cores)")
            n = len(events_df)
            rp = events_df["RankPos"].astype(int)
            overall = {
                "HitEvents": int(n),
...
def position_percentile_map(df_rankpos: pd.DataFrame) -> pd.DataFrame:
    ...
    if df_rankpos is None or df_rankpos.empty:
        return pd.DataFrame(columns=[
            "RankPos", "HitCount", "HitCountPctile", "HitSharePct", "CumuHitSharePct"
        ])
    ...
    # (robust to NaNs / non-numeric RankPos that can appear when upstream parsing fails)
    pos["RankPos"] = pd.to_numeric(pos["RankPos"], errors="coerce")
    pos["HitCount"] = pd.to_numeric(pos["HitCount"], errors="coerce").fillna(0.0)
    pos = pos.dropna(subset=["RankPos"]).copy()
    if pos.empty:
        return pd.DataFrame(columns=[
            "RankPos", "HitCount", "HitCountPctile", "HitSharePct", "CumuHitSharePct"
        ])
    ...
    total = float(pos["HitCount"].sum()) if float(pos["HitCount"].sum()) != 0.0 else 1.0
    pos["HitSharePct"] = (pos["HitCount"] / total) * 100.0
    pos["CumuHitSharePct"] = pos["HitSharePct"].cumsum()

    # Back-compat: some UI blocks expect a generic percentile strength field name.
    # Keep the primary percentile column as HitCountPctile; expose PctStrength as an alias.
    pos["PctStrength"] = pos["HitCountPctile"]

    # Back-compat aliases used in older UI text
...
def get_position_percentiles_cached(core: str, window_days: int, stream_stats: pd.DataFrame) -> pd.DataFrame:
    ...
    # position_percentile_map returns a single DataFrame (legacy builds returned (df, meta)).
...
def bucket_recommendations(stream_stats: pd.DataFrame, cfg: RankConfig) -> dict:
    """Return stream buckets for a core: Top 12 by RankPos, Due 8 by DuePressure within a rank band, and Combined."""
    if stream_stats is None or stream_stats.empty:
        return {
            "Top12": [],
            "Due8": [],
            "Combined": [],
    ...
    for _col in ["RankPos", "HitsPerWeek", "DaysSinceLastHit", "DuePressure"]:
        if _col in df.columns:
            df[_col] = pd.to_numeric(df[_col], errors="coerce")

    # Fallback: if RankPos is missing/blank (e.g., older cached schema), compute a deterministic RankPos.
    if ("RankPos" not in df.columns) or df["RankPos"].isna().all():
        # Prefer higher HitsPerWeek; use DaysSinceLastHit as secondary tie-breaker.
        if "HitsPerWeek" in df.columns:
            df["HitsPerWeek"] = pd.to_numeric(df["HitsPerWeek"], errors="coerce")
        else:
            df["HitsPerWeek"] = 0.0
        if "DaysSinceLastHit" in df.columns:
            df["DaysSinceLastHit"] = pd.to_numeric(df["DaysSinceLastHit"], errors="coerce")
        else:
            df["DaysSinceLastHit"] = 0.0
        df = df.sort_values(["HitsPerWeek", "DaysSinceLastHit"], ascending=[False, False], kind="mergesort")
        df["RankPos"] = range(1, len(df) + 1)

    # Fallback: if DuePressure is missing/blank, use DaysSinceLastHit as a due proxy.
    if ("DuePressure" not in df.columns) or df["DuePressure"].isna().all():
        if "DaysSinceLastHit" in df.columns:
            df["DuePressure"] = pd.to_numeric(df["DaysSinceLastHit"], errors="coerce").fillna(0)
        else:
            df["DuePressure"] = 0

    base_n = int(getattr(cfg, "top_base", 12) or 12)
    due_n = int(getattr(cfg, "top_due", 8) or 8)
    due_from = int(getattr(cfg, "due_from_rank", 13) or 13)
    due_to = int(getattr(cfg, "due_to_rank", 60) or 60)
    combined_max = int(getattr(cfg, "combined_max", 20) or 20)

    base_df = df.nsmallest(base_n, "RankPos")
    base_streams = base_df["Stream"].tolist() if "Stream" in base_df.columns else []

    band_df = df[(df["RankPos"] >= due_from) & (df["RankPos"] <= due_to)] if "RankPos" in df.columns else df.copy()
    due_df = band_df.nlargest(due_n, "DuePressure") if "DuePressure" in band_df.columns else band_df.head(due_n)
    due_streams = due_df["Stream"].tolist() if "Stream" in due_df.columns else []

    combined = []
    for s in base_streams + due_streams:
        if s not in combined:
            combined.append(s)
        if len(combined) >= combined_max:
            break
    ...
    return {
        ...
        "Due8": due_streams,
        "Combined": combined,
        # Aliases for older codepaths / helper utilities (do not remove)
        "base_top": base_streams,
        "due_top": due_streams,
        "combined": combined,
        # back-compat metadata (some builds read these keys)
        "top_n": base_n,
        "base_top_n": base_n,
        "due_top_n": due_n,
        "due_ranks": (due_from, due_to),
    }
...
def run_system_check() -> "pd.DataFrame":
    """
    Regression-proof checklist: lightweight runtime checks that confirm required
    modules and key behaviors are present. This is intentionally fast.
    """
    checks = []
    ...
    # --- Presence checks (functions/modules) ---
    add("Backtest module present", callable(globals().get("render_backtest")), "render_backtest() is defined")
    add(
        "Straights module present",
        callable(globals().get("render_straights_shortlist")) and callable(globals().get("unique_straights_for_box")),
        "render_straights_shortlist() + unique_straights_for_box() defined",
    )
    add("Seed traits loader present", callable(globals().get("load_seed_traits_csv")), "load_seed_traits_csv() is defined")
    add("Cadence scoring present", callable(globals().get("cadence_score")), "cadence_score() is defined")
    add(
        "Northern Star bucket logic present",
        callable(globals().get("build_northern_star_buckets_for_core")) and callable(globals().get("compute_northern_star_bucket_meta")),
        "Northern Star bucket helpers are defined",
    )

    # --- Percentile map logic should execute ---
    try:
        _tmp = pd.DataFrame({"RankPos": [1, 2, 2, 10, 10, 10], "HitCount": [1, 1, 1, 1, 1, 1]})
        _pm = position_percentile_map(_tmp)
        ok = isinstance(_pm, pd.DataFrame) and "RankPos" in _pm.columns
        add("Percentile map function works", ok, f"rows={len(_pm) if ok else 'n/a'}")
    except Exception as e:
        add("Percentile map function works", False, f"{type(e).__name__}: {e}")

    # --- Source-based checks (catch known legacy bug patterns) ---
    src = ""
    try:
    ...
    if src:
        add(
            "No legacy percentile-unpack bug",
            (("pos_map" + ", _ = " + "position_percentile_map(") not in src),
            "Ensures we do not unpack a single-return percentile map",
        )
        add(
            "Master checklist embedded",
            ("MASTER_CHECKLIST_MD" in src and "V51_INTEGRATION_PASS_MD" in src),
            "Embedded requirement checklists are present",
        )
    else:
        add("No legacy percentile-unpack bug", True, "source unavailable (skipped)")
        add("Master checklist embedded", True, "source unavailable (skipped)")

    # --- Session state expectations (non-fatal) ---
    add(
        "Multi-core selection state exists",
        ("selected_cores" in st.session_state and isinstance(st.session_state.get("selected_cores"), list)),
        f"selected_cores={len(st.session_state.get('selected_cores', [])) if isinstance(st.session_state.get('selected_cores'), list) else 'n/a'}",
    )
...
with st.sidebar:
    ...
    st.caption("Soft weighting only (never eliminates). Uses the previous winner per stream (from the 24h map file).")
    ...
    with st.expander("Build checklist (do not omit)", expanded=False):
        ...
        st.caption("This confirms prerequisites (core list + current selections) when a section looks empty.")
...
st.divider()
straights_opt = st.checkbox("Generate straights shortlist (optional last)", value=bool(st.session_state.get("do_straights", False)), key="do_straights")
...
with st.sidebar.expander("Master checklist (requirements)", expanded=False):
    st.markdown(MASTER_CHECKLIST_MD)
    st.markdown(V51_INTEGRATION_PASS_MD)

# Sidebar status summary (quick PASS/FAIL view)
try:
    _sc_df_sidebar = run_system_check()
    _pass_ct = int((_sc_df_sidebar["Status"] == "PASS ✅").sum())
    _total_ct = int(len(_sc_df_sidebar))
    st.sidebar.caption(f"System Check: {_pass_ct}/{_total_ct} PASS ✅")
except Exception:
    pass
...
tab_labels = [
    "Northern Lights (Master playlist)",
    "Northern Star",
    "Core view",
    "Backtest (optional)",
    "System Check",
]
_t_nl, _t_ns, _t_core, _t_bt, _t_sys = st.tabs(tab_labels)
...
with _t_nl:
    ...
    # Backward-compatible alias (some sections expect `selected_cores`)
    selected_cores = list(cores_for_cache)
    try:
        st.session_state["selected_cores"] = list(selected_cores)
    except Exception:
        pass
    ...
    if not cores_for_cache:
        ...
        if nl_use_all_cores:
            ...
            for _c in cores_for_cache:
                ss, _pos, _meta = _load_baseline_from_disk(_c, cfg.window_days, expected_last_date=expected_last)
                if ss is None or ss.empty:
                    missing.append(_c)
                else:
                    stats_by_core[_c] = ss
        ...
        if not rows:
            ...
            with st.expander("Northern Star percentile map (playlist positions)", expanded=False):
                try:
                    ...
                    _pos = position_percentile_map(_best[["RankPos", "HitsWindow"]].copy())
                    ...
                    st.dataframe(_pos, use_container_width=True, height=320)
...
with _t_ns:
    ...
    if not ns_cores:
        ...
        if global_map is not None:
            st.dataframe(global_map, use_container_width=True)
        if missing:
            st.error("Missing baseline caches for these cores (global map is cache-only): " + ", ".join(missing))
            st.caption("Build caches in the Cache Builder section, then rerun.")
        elif global_map is None:
            st.warning("Global percentile map unavailable (no cached baselines found). Build caches, then rerun.")

    st.divider()
    st.subheader("Per-core Northern Star scoring (SeedTraits + Cadence, soft)")
    view_core_ns = st.selectbox("Core (Northern Star view)", options=list(dict.fromkeys([view_core] + list(selected_cores))), key="view_core_ns")
    core_key_ns = canonical_core_key(view_core_ns)

    try:
        _exclude_md_ns = bool(st.session_state.get('exclude_md', True))
        stats_ns = compute_stream_stats(df_all, core_key_ns, window_days=cfg.window_days, exclude_md=_exclude_md_ns)
    except Exception as e:
    ...
    if stats_ns is not None and not stats_ns.empty:
        ...
        st.dataframe(ns_df.head(50), use_container_width=True)
        ...
        with st.expander("Per-core RankPos percentile map (position-based)"):
            st.dataframe(pos_map_ns, use_container_width=True)
        ...
        if st.session_state.get("r1", True) or st.session_state.get("r2", True) or st.session_state.get("r3", True) or st.session_state.get("r4", True):
            rare = evaluate_rare_engine(core_key_ns, df_all, cfg.window_days)
        ...
        if st.session_state.get("q1", True) or st.session_state.get("q2", True) or st.session_state.get("q3", True) or st.session_state.get("q4", True):
            ultra = evaluate_ultra_rare_engine(core_key_ns, df_all, cfg.window_days)
            st.dataframe(ultra, use_container_width=True)
        else:
        ...
        with st.expander("Seed Traits matches (debug)"):
            ...
            if matches:
                st.dataframe(pd.DataFrame(matches, columns=["trait","value","lift","sign"]), use_container_width=True)
            else:
...
# Guarded to avoid crashes if `pos_pct` is not in scope (depends on which tab executes first).
# --- System Check (regression proof) ---
with _t_sys:
    st.header("System Check (Regression Proof)")
    st.caption("This page runs lightweight validations so missing features/regressions are visible immediately.")

    _sc_df = run_system_check()
    st.dataframe(_sc_df, use_container_width=True, hide_index=True)

    _fails = _sc_df[_sc_df["Status"].str.contains("FAIL")]
    if len(_fails):
        st.error(f"{len(_fails)} checks failed. Fix these before deploying.")
    else:
        st.success("All checks passed ✅")

    with st.expander("Embedded master checklists", expanded=False):
        st.markdown(MASTER_CHECKLIST_MD)
        st.markdown(V51_INTEGRATION_PASS_MD)

    with st.expander("Debug aids (percentile caches)", expanded=False):
        st.caption("Shows cached percentile maps if they exist on disk (helps confirm the correct version is deployed).")
        try:
            _window_days = int(st.session_state.get("window_days", 180))
        except Exception:
            _window_days = 180

        _allcores = get_allcores_rankpos_pctmap(cache_only=True)
        if isinstance(_allcores, pd.DataFrame) and len(_allcores):
            st.subheader("Global / All-cores position percentile map")
            st.dataframe(_allcores, use_container_width=True, hide_index=True)
        else:
            st.warning("All-cores percentile map cache not found yet. Build cache to enable this map.")

        _sel = st.session_state.get("selected_cores", [])
        if isinstance(_sel, list) and _sel:
            _core_for_debug = st.selectbox("Pick a core to inspect its cached map", _sel, index=0, key="sys_core_debug")
            _core_pm = _load_pctmap_from_disk(f"CORE_{_core_for_debug}", _window_days, CACHE_DIR)
            if isinstance(_core_pm, pd.DataFrame) and len(_core_pm):
                st.subheader(f"Per-core position percentile map: {_core_for_debug}")
                st.dataframe(_core_pm, use_container_width=True, hide_index=True)
            else:
                st.info("Per-core percentile cache not found yet (build cache for this core).")
