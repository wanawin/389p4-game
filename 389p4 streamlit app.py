...
# Baseline cache tools (optional)
st.subheader("Baseline cache builder")
st.caption(
    "This builds a local on‑disk cache for the selected cores so daily runs stay fast. "
    "It also lets the app show you the last date currently contained in the cached baseline."
)
build_both = st.checkbox("Build cache for both windows (180 & 365)", value=False)
if last_all is not None and not pd.isna(last_all):
    _streams = int(df_all["Stream"].nunique())
    cache_key = _cache_key(pd.to_datetime(last_all), int(len(df_all)), _streams, bool(exclude_md), int(365 if build_both else window_days), cores)
    meta_path = DISK_CACHE_DIR / f"baseline_{cache_key}_meta"
    meta = _read_meta(meta_path)
    if meta:
        st.info(f"Cached baseline last built: {meta.get('built_at','?')} | max date: {meta.get('max_date','?')} | cores: {meta.get('cores','?')}")
    else:
        st.warning("No baseline cache found for this selection yet.")

    if st.button("Build / Refresh baseline cache now"):
        windows_to_build = [180, 365] if build_both else [int(window_days)]
        built = {"windows": [], "cores": 0, "files": []}
        with st.spinner("Building baseline cache (this can take a bit on first run)…"):
            max_date = pd.to_datetime(last_all)
            for w in windows_to_build:
                recent_cut = max_date - pd.Timedelta(days=int(w))
                df_recent = df_all[df_all["Date"] >= recent_cut].copy()
                # Collect tables
                all_stream_rows = []
                all_pos_rows = []
                for core in cores:
                    members_aabc = generate_family_members_for_core(core, "AABC")
                    df_s = compute_stream_summary_by_members(df_recent, members_aabc, structure="AABC")
                    df_s["core"] = core
                    df_s["window_days"] = int(w)
                    all_stream_rows.append(df_s)

                    pos_map = compute_position_percentiles(df_s)
                    pos_map["core"] = core
                    pos_map["window_days"] = int(w)
                    all_pos_rows.append(pos_map)

                df_stream_all = pd.concat(all_stream_rows, ignore_index=True) if all_stream_rows else pd.DataFrame()
                df_pos_all = pd.concat(all_pos_rows, ignore_index=True) if all_pos_rows else pd.DataFrame()

                # key per window
                ck = _cache_key(max_date, int(len(df_all)), _streams, bool(exclude_md), int(w), cores)
                base_path = DISK_CACHE_DIR / f"baseline_{ck}"
                ok1, p1 = _safe_write_table(df_stream_all, base_path.with_name(base_path.name + "_aabc_streams"))
                ok2, p2 = _safe_write_table(df_pos_all, base_path.with_name(base_path.name + "_aabc_pospct"))

                built["windows"].append(int(w))
                built["cores"] = len(cores)
                built["files"].append(p1)
                built["files"].append(p2)
                ...
                _write_meta(DISK_CACHE_DIR / f"baseline_{ck}_meta", meta)

        st.success(f"Baseline cache built for windows {built['windows']} and {built['cores']} core(s).")
        st.caption("Saved tables: " + " | ".join([f for f in built["files"] if f]))
...
if last_24h is not None and not pd.isna(last_24h):
    st.sidebar.caption(f"24h map most recent date: {pd.to_datetime(last_24h).date()}  (age: {_age_days(last_24h)} days)")
else:
...
with colC:
    st.markdown("**Date span**")
    st.caption(f"{df_all['Date'].min().date()} → {df_all['Date'].max().date()}")
