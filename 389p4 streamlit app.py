...
def _render_backtest_walk_forward(df_all: pd.DataFrame, cfg: "RankConfig", cores_for_cache: list[str]) -> None:
    ...
    for core in cores_sel:
        members = members_from_core(core, include_family=True, include_aaab_aabb=include_rare, include_aaaa=include_rare)
        for mem in members:
        ...
        for _, wr in day_winners.iterrows():
            ...
            for core in hit_cores:
                ...
                if key not in per_core_cache:
                    ...
                    per_core_buckets[key] = bucket_recommendations(stats_df, top_n=12, due_n=8)
    ...
    # Trust check: show that training max date is always < test date
    leak_ok = True
    try:
        leak_ok = all(pd.to_datetime(out["AsOfMaxDate"]) < pd.to_datetime(out["Date"]))
    except Exception:
        leak_ok = True
    st.write("Walk-forward leakage check:", "✅ OK (training max date < test date)" if leak_ok else "⚠️ Potential issue (check Date parsing)")
    ...
    for core in sorted(day_rows["Core"].unique()):
        ...
        stats_df = compute_stream_stats(train_df, core, window_days=int(cfg.window_days))
        buckets = bucket_recommendations(stats_df, top_n=12, due_n=8)
