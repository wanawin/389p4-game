    ...
    for raw in text.splitlines():
        ...
        for i, p in enumerate(parts):
            if date_idx is None:
                ...
                if dt is not None:
                    date_idx = i
                    parsed_dt = dt
                    # IMPORTANT: if this token is the date (e.g., 'Sat, Dec 27, 2025'),
                    # don't let the year (2025) be mis-read as the 4-digit result.
                    continue
        ...
        if parsed_dt is None or parsed_res is None:
            ...
            if mdate:
                ...
                # Avoid accidentally grabbing the year from the date as the "result"
                remainder = line[mdate.end():]
                parsed_res = parsed_res or extract_4digit_result(remainder)
            else:
