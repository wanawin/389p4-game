...
# View core (dropdown)
# Local alias used by dropdowns and widgets (avoid NameError/regressions)
...
# Local alias used later in Core view/Backtest blocks
cores_for_cache = list(st.session_state.get("cores_for_cache", []))
...
with _t_core:
    ...
    # Ensure a local alias exists even if upstream widgets haven't run yet
    cores_for_cache = list(st.session_state.get("cores_for_cache", []))
    if not cores_for_cache:
    ...
    def _render_one_core(core_id: str):
        ...
        # Buckets (Northern Star)
        rec = bucket_recommendations(stats_df, cfg)
        top_bucket = rec.get("base_top", [])
        due_bucket = rec.get("due_top", [])
        combined_bucket = rec.get("combined", [])
