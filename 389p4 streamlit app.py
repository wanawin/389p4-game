...
def build_allcores_rankpos_pctmap(
    ...
    for core in cores_list:
        if cache_only:
            ss, _ld = _load_baseline_from_disk(core, window_days, expected_last_date=expected_last_date)
            ...
            ss = compute_stream_stats_for_core(
                df_all=df_all,
                core=core,
                cfg=cfg,
                stream_rankings_df=stream_rankings_df,
                family_counts_df=family_counts_df,
                struct_counts_df=struct_counts_df,
            )
    ...
    if not cores_for_cache:
        ...
        if nl_use_all_cores:
            ...
            for _c in cores_for_cache:
                ss, _meta = _load_baseline_from_disk(_c, cfg.window_days, expected_last_date=expected_last)
                if ss is None or ss.empty:
                    missing.append(_c)
                else:
                    stats_by_core[_c] = ss
