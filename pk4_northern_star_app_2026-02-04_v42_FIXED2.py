...
# always ensure default core is present
if default_core not in st.session_state["cores_for_cache"]:
    st.session_state["cores_for_cache"] = [default_core] + [c for c in st.session_state["cores_for_cache"] if c != default_core]

if "view_core" not in st.session_state:
    st.session_state["view_core"] = default_core
else:
    st.session_state["view_core"] = str(st.session_state["view_core"]).zfill(3)
    if st.session_state["view_core"] not in available_cores:
        st.session_state["view_core"] = default_core


def _sync_cores_for_cache():
    """Ensure multi-core selection stays valid and always includes the default core."""
    sel = st.session_state.get("cores_for_cache", []) or []
    sel = [str(c).zfill(3) for c in sel]
    # keep only valid options
    sel = [c for c in sel if c in available_cores]
    # enforce default core always included
    if default_core not in sel:
        sel = [default_core] + sel
    # de-dupe while preserving order
    seen = set()
    out = []
    for c in sel:
        if c not in seen:
            seen.add(c)
            out.append(c)
    st.session_state["cores_for_cache"] = out
...
view_core = st.selectbox(
    ...
    index=available_cores.index(st.session_state["view_core"]),
    key="view_core",
)
...
st.multiselect(
    ...
    options=available_cores,
    key="cores_for_cache",
    on_change=_sync_cores_for_cache,
)

# Run sync once per run as well (covers first render / option list changes)
_sync_cores_for_cache()
