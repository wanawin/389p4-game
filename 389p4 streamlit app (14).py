...
# Canonical core list used by dropdowns and cache tools
cores = list(available_cores)
...
with _t_nl:
    ...
    if not cores_for_cache:
        ...
        if not rows:
            ...
            st.caption('Playlist percentile map section below (if you do not see it, the deployed file is not updated).')

            # Playlist percentile map (tie-breaker)
            with st.expander("Playlist percentile map (tie-breaker)", expanded=True):
                try:
                    _tmp = nl_df.copy()
                    # Percentile ranks for quick hotspot visibility
                    _tmp["UniversalScorePctile"] = (_tmp["UniversalScore"].rank(pct=True) * 100.0).round(1)
                    _tmp["PctStrengthPctile"] = (_tmp["PctStrength"].rank(pct=True) * 100.0).round(1)
                    # Simple bands for UniversalScore
                    def _band(p):
                        ...
                        return "Bottom 50%"
                    _tmp["UniversalBand"] = _tmp["UniversalScorePctile"].apply(_band)

                    # Band summary
                    band_counts = (_tmp["UniversalBand"].value_counts()
                                   .reindex(["Top 1%","Top 5%","Top 10%","Top 20%","Top 50%","Bottom 50%"])
                                   .fillna(0).astype(int).reset_index())
                    band_counts.columns = ["Band", "Count"]
                    st.caption("UniversalScore percentile bands across the master playlist.")
                    st.dataframe(band_counts, use_container_width=True, height=230)

                    st.caption("Full playlist with percentiles (sort/filter in-table).")
                    st.dataframe(
                        _tmp[["Rank","Core","Stream","BucketPick","UniversalScore","UniversalScorePctile","PctStrength","PctStrengthPctile","HitsPerWeek","DuePressure","Hits","DaysSinceLastHit","UniversalBand"]],
                        use_container_width=True,
                        height=520,
                    )
                    st.caption("Tip: PctStrength is a soft tie-breaker (position-percentile strength) when UniversalScore is close.")
                except Exception as e:
                    st.error(f"Could not build playlist percentile map: {e}")
...
with _t_core:
    ...
    if show_tabs:
        ...
        _render_one_core(str(view_core))
