...
class RankConfig:
    ...
    @property
    def top_n(self) -> int:
        """Total bucket size (Top + Due). Kept for backward compatibility."""
        return int(self.top_base) + int(self.top_due)
...
# Initialize multiselect state once (avoid Streamlit warning about default + session_state)
if "cores_for_cache_ms" not in st.session_state or not st.session_state.cores_for_cache_ms:
...
cores_for_cache = [str(c).zfill(3) for c in (st.session_state.cores_for_cache_ms or [])]
...
# Persist
st.session_state.cores_for_cache = list(cores_for_cache)
...
def compute_buckets(stream_stats: pd.DataFrame, cfg: RankConfig):
    """
    Build Northern Star buckets for a given core.

    Returns (top_bucket, due_bucket, combined_bucket) where each bucket is a list[str] of Stream names.
    - top_bucket: Top cfg.top_base by HitsPerWeek (ties: HitsWindow, then DaysSinceLastHit)
    - due_bucket: From ranks cfg.due_from_rank..cfg.due_to_rank in the base ranking, take Top cfg.top_due by DaysSinceLastHit
    - combined_bucket: top_bucket + due_bucket (deduped, preserving order)
    """
    if stream_stats is None or stream_stats.empty:
        return [], [], []

    df = stream_stats.copy()
    # Ensure required columns exist (older cached frames may miss some).
    for col, default in [
        ("HitsPerWeek", 0.0),
        ("HitsWindow", 0),
        ("DaysSinceLastHit", 10**9),
        ("Stream", ""),
    ]:
        if col not in df.columns:
            df[col] = default

    # Base ranking used for both 'Top' and to define the rank window for Due.
    df = df.sort_values([
        "HitsPerWeek",
        "HitsWindow",
        "DaysSinceLastHit",
    ...
    df["RankPos"] = df.index + 1

    top_n = int(getattr(cfg, "top_base", 12))
    top_bucket = df.head(top_n)["Stream"].astype(str).tolist()

    r1 = int(getattr(cfg, "due_from_rank", 13))
    r2 = int(getattr(cfg, "due_to_rank", 60))
    due_n = int(getattr(cfg, "top_due", 8))
    due_slice = df[(df["RankPos"] >= r1) & (df["RankPos"] <= r2)].copy()
    # 'Due' sort: most days since last hit first, with some stability on strength.
    due_slice = due_slice.sort_values(["DaysSinceLastHit", "HitsPerWeek", "HitsWindow", "Stream"],
                                   ascending=[False, False, False, True])
    due_bucket = due_slice.head(due_n)["Stream"].astype(str).tolist()

    combined_bucket = []
    seen = set()
    for s in top_bucket + due_bucket:
        if s not in seen:
            combined_bucket.append(s)
            seen.add(s)

    return top_bucket, due_bucket, combined_bucket
        ...
        for core in cores_for_cache:
            ...
            for stream in stats_df["Stream"].astype(str).tolist():
                meta = build_northern_star_buckets(
                    ...
                    stream=stream,
                    top_n=cfg.top_n,
                    due_ranks=(cfg.due_from, cfg.due_to),
