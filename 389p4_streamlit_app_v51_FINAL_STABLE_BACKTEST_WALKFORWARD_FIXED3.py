...
def bucket_recommendations(
    ...
    *,
    ...
    if stream_stats is None or len(stream_stats) == 0:
        empty = pd.DataFrame()
        return {
    ...
    base_df = stream_stats.sort_values("BaseScoreRank", ascending=True).head(base_n)
    due_pool = stream_stats[
        (stream_stats["BaseScoreRank"] >= due_lo) & (stream_stats["BaseScoreRank"] <= due_hi)
