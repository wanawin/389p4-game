...
def _parse_csv_flexible(text: str) -> Optional[pd.DataFrame]:
    """Attempt CSV/TSV parsing using pandas' delimiter sniffing."""
    try:
...
def _parse_txt_lines(text: str) -> pd.DataFrame:
    ...
    for raw_line in text.splitlines():
        ...
        # Prefer tab split, else 2+ spaces, else single comma split fallback
        parts: List[str]
        if "\t" in line:
...
def load_history(uploaded, filter_families: bool = True) -> pd.DataFrame:
    ...
    if name.endswith(".csv"):
        ...
        df = _parse_csv_flexible(text) or _parse_txt_lines(text)
    else:
...
def straight_ranking_for_stream(df_stream: pd.DataFrame,
                               ...
                               asof: pd.Timestamp | None = None)[0] -> tuple[pd.DataFrame, dict]:
