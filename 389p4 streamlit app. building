...
def get_position_percentiles_cached(core: str, window_days: int, stream_stats: pd.DataFrame) -> pd.DataFrame:
    ...
    # status breadcrumb for UI checklist
    try:
        st.session_state["_pctmap_last"] = {
            "core": str(core).zfill(3),
            "window_days": int(window_days),
            "rows": int(len(pos_map)) if hasattr(pos_map, "__len__") else None,
...
def bucket_recommendations(stream_stats: pd.DataFrame, cfg: RankConfig) -> dict:
    ...
    return {"Top12": base_streams, "Due8": due_streams, "Combined": combined}
...
""")
        ...
        # Live status (prevents silent regressions)
        def _ok(flag: bool) -> str:
            return "✅" if bool(flag) else "❌"

        _selected = st.session_state.get("cores_for_cache_ms", st.session_state.get("cores_for_cache", [])) or []
        _selected = [str(c).zfill(3) for c in _selected]
        _pct_last = st.session_state.get("_pctmap_last")

        _pct_desc = ""
        try:
            if isinstance(_pct_last, dict):
                _pct_desc = f"(last: core {_pct_last.get('core')} / {_pct_last.get('window_days')}d / rows {_pct_last.get('rows')})"
        except Exception:
            _pct_desc = ""
        ...
        _disk_files = 0
        try:
            _disk_files = len(list(DISK_CACHE_DIR.glob('*.*')))
        except Exception:
            _disk_files = 0

        st.markdown("### Live status (PASS/FAIL)")
        st.markdown(
            f"- **DCORES_CNT** Daily tracked Doubles cores hardcoded: **{len(CORE_PRESETS)}** (target ≈ 57–63)\n"
            f"- **MCSEL** Multi-core selected right now: **{len(_selected)}** core(s)\n"
            f"- **PCTMAP_RUN** Percentile map computed this session: {_ok(bool(_pct_last))} {_pct_desc}\n"
            f"- **SM_PURGE** Disk cache folder readable: {_ok(_disk_files >= 0)} (files: {_disk_files})\n"
        )
        st.caption("If something shows ❌, it usually means a required input isn't loaded yet (history file, core selection, or maps not computed).")
