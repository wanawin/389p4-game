...
# Initialize multi-core widget state once (avoid default+session_state warning)
if "cores_for_cache_ms" not in st.session_state:
    st.session_state.cores_for_cache_ms = [str(c).zfill(3) for c in (st.session_state.cores_for_cache or [])]
# Ensure currently viewed core is included
ms = [str(c).zfill(3) for c in (st.session_state.cores_for_cache_ms or [])]
if vc not in ms:
    ms = ms + [vc]
    st.session_state.cores_for_cache_ms = ms
