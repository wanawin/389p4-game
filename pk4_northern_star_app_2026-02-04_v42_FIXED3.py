...
# Derive available/default cores from config. Keep everything zfilled.
available_cores = [str(c).zfill(3) for c in getattr(cfg, "available_cores", [])]
default_core = str(getattr(cfg, "default_core", "389")).zfill(3)
if not available_cores:
    available_cores = [default_core]
elif default_core not in available_cores:
    available_cores = [default_core] + available_cores
else:
    # Ensure default core is first (keeps UI stable)
    available_cores = [default_core] + [c for c in available_cores if c != default_core]

def _normalize_cores_for_cache(sel):
    """Normalize multi-core selection: keep valid, zfill, dedupe, and always include default core."""
    sel = sel or []
    out = []
    seen = set()
    for c in sel:
        cc = str(c).zfill(3)
        if cc in available_cores and cc not in seen:
            out.append(cc)
            seen.add(cc)
    if default_core not in seen:
        out = [default_core] + out
    return out

# Canonical session-state values (NOT widget keys)
st.session_state["view_core"] = str(st.session_state.get("view_core", default_core)).zfill(3)
if st.session_state["view_core"] not in available_cores:
    st.session_state["view_core"] = default_core

st.session_state["cores_for_cache"] = _normalize_cores_for_cache(
    st.session_state.get("cores_for_cache", [default_core])
)

# Widget mirrors (safe: set BEFORE widgets render)
st.session_state["view_core_widget"] = st.session_state["view_core"]
st.session_state["cores_for_cache_widget"] = list(st.session_state["cores_for_cache"])

view_core_widget = st.selectbox(
    "View core (dropdown)",
    options=available_cores,
    index=available_cores.index(st.session_state["view_core_widget"]),
    key="view_core_widget",
)

# Keep canonical in sync (safe: canonical key differs from widget key)
st.session_state["view_core"] = str(view_core_widget).zfill(3)

# Multi-core selection for cache/batch tools (chips should reflect all selected cores)
cores_widget = st.multiselect(
    "Cores to include for cache building / batch tools",
    options=available_cores,
    key="cores_for_cache_widget",
)

# Keep canonical in sync (safe: canonical key differs from widget key)
st.session_state["cores_for_cache"] = _normalize_cores_for_cache(cores_widget)
