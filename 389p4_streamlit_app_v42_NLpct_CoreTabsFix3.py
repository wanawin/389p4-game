...
view_core = st.selectbox("View core (dropdown)", options=available_cores, index=available_cores.index(default_core), key="view_core")

# Keep cache/batch selection in sync with the view dropdown (preserves prior behavior)
try:
    vc = str(view_core).zfill(3)
except Exception:
    vc = default_core

# Initialize the canonical cache list
if "cores_for_cache" not in st.session_state or not st.session_state.cores_for_cache:
    st.session_state.cores_for_cache = [default_core]
st.session_state.cores_for_cache = [str(c).zfill(3) for c in (st.session_state.cores_for_cache or [])]
if vc not in st.session_state.cores_for_cache:
    st.session_state.cores_for_cache = st.session_state.cores_for_cache + [vc]

# Initialize multiselect state once (avoids StreamlitAPIException about default+session_state)
if "cores_for_cache_ms" not in st.session_state:
    st.session_state.cores_for_cache_ms = list(st.session_state.cores_for_cache)

# Multi-core selection for cache/batch tools (widget owns its session_state key)
cores_for_cache = st.multiselect(
    "Cores to include for cache building / batch tools",
    options=available_cores,
    key="cores_for_cache_ms",
)

# Keep at least one core selected
if not cores_for_cache:
    cores_for_cache = [default_core]

# Normalize and persist (canonical list used everywhere else)
cores_for_cache = [str(c).zfill(3) for c in cores_for_cache]
st.session_state.cores_for_cache = cores_for_cache
...
def _input_sig(u):
    """Lightweight signature to detect when uploaded inputs change."""
    try:
        return (getattr(u, "name", None), getattr(u, "size", None))
    except Exception:
        return None

_cur_sig = (_input_sig(master_file), _input_sig(map24_file))
_prev_sig = st.session_state.get("_input_sig_prev")
force_autorun_once = False
if master_file is not None:
    if _prev_sig is None:
        st.session_state["_input_sig_prev"] = _cur_sig
        force_autorun_once = True
    elif _cur_sig != _prev_sig:
        st.session_state["_input_sig_prev"] = _cur_sig
        force_autorun_once = True
...
with c2:
    nl_autorun = st.checkbox("Auto-run when inputs change", key="nl_autorun", value=True, help="Runs automatically when you change controls above. If off, use the Run button.")
    ...
    cores = st.session_state.get('cores_for_cache', [str(view_core).zfill(3)])
    compute_now = nl_autorun or run_now or force_autorun_once
    ...
    if compute_now:
        ...
        if isinstance(cores, str):
            cores = [cores]
        cores = [str(c).zfill(3) for c in (cores or [])]
        if not cores:
            cores = [str(view_core).zfill(3)]
        ...
        for idx, core in enumerate(cores):
            ...
            for _, r in stats.iterrows():
                ...
                dslh = (_safe_int(buckets.get("DaysSinceLastHit", 0)) or 0)
    ...
    if df_master_show is None:
        ...
        # --- Member-level scored plays (box members) (NO scoring changes) ---
        # --- Display-only explainer: why a row is ranked where it is (NO scoring changes) ---
        with st.expander("Northern Lights: Why is this ranked here? (display only)", expanded=False):
            st.caption("Display-only. Explains the components already used in MasterScore. No scoring/model changes.")
            try:
                _nl_df = df_master_show.copy()
                _nl_df["_label"] = _nl_df.apply(lambda r: f"#{int(r['Rank'])} | Core {str(int(r['Core'])).zfill(3)} | {str(r['Stream'])}", axis=1)
                _labels = _nl_df["_label"].tolist()
                _pick = st.selectbox("Pick a row to explain", options=_labels, index=0, key="nl_explain_pick")
                _row = _nl_df[_nl_df["_label"] == _pick].iloc[0]

                _core = str(int(_row["Core"])).zfill(3)
                _stream = str(_row["Stream"])
                _master = float(_row.get("MasterScore", 0.0) or 0.0)
                _rec = float(_row.get("HitsPerWeek", 0.0) or 0.0)
                _due = float(_row.get("DuePressure", 0.0) or 0.0)
                _pos = float(_row.get("PosStrength", 0.0) or 0.0)
                _w = float(getattr(cfg, "pos_strength_weight", 1.0) or 1.0)

                # Pull the underlying per-core stream stats row (for RankPos / HitsWindow context)
                _stats = get_stream_stats_cached(_core, int(cfg.window_days), df_all=df_all, last_date=last_all)
                _stats_row = _stats[_stats["Stream"].astype(str) == _stream]
                if not _stats_row.empty:
                    _sr = _stats_row.iloc[0]
                    _rankpos = int(_sr.get("RankPos", -1)) if pd.notna(_sr.get("RankPos", np.nan)) else -1
                    _hitsw = int(_sr.get("HitsWindow", 0) or 0)
                    _dslh = int(_sr.get("DaysSinceLastHit", 0) or 0)
                else:
                    _rankpos, _hitsw, _dslh = -1, int(_row.get("Hits", 0) or 0), int(_row.get("DaysSinceLastHit", 0) or 0)

                # Recompute bucket membership flags exactly as used upstream (for explanation only)
                _b = build_northern_star_buckets(
                    stats_df=_stats,
                    stream=_stream,
                    top_n=int(cfg.top12),
                    due_ranks=(int(cfg.due_ranks[0]), int(cfg.due_ranks[1])),
                ...
                _bucket = str(_b.get("Top12", ""))  # "Top12" or "Due" or ""
                _bucket_pick = str(_b.get("BucketPick", ""))  # "BASE" or "DUE" or ""
                    ...
                    st.write(
                        {
                            ...
                            "Stream": _stream,
                            "BucketLabel": _bucket,
                            "BucketPick": _bucket_pick,
                            "RankPos (within core)": _rankpos,
                            "HitsWindow": _hitsw,
                            "DaysSinceLastHit": _dslh,
                        }
                    )

                with right:
                    st.markdown("**MasterScore breakdown**")
                    st.write(
                        {
                            "MasterScore": round(_master, 4),
                            "RecentStrength = HitsPerWeek * 100": round(_rec * 100.0, 4),
                            "DuePressure": round(_due, 4),
                            f"PosStrength * weight ({_w})": round(_pos * _w, 4),
                        }
                    )
                    st.caption("Formula: MasterScore = (HitsPerWeek * 100) + DuePressure + (PosStrength * pos_strength_weight)")

            except Exception as e:
                st.error(f"Could not build explainer panel: {e}")



    with st.expander("Northern Lights: position percentile tables (display only)", expanded=False):
        st.caption("Display-only. Shows the per-core RankPos percentile table used for diagnostics. No scoring/model changes.")
        core_opts = [str(c).zfill(3) for c in (cores or [])]
        if not core_opts:
            core_opts = [str(view_core).zfill(3)]
        core_pick = st.selectbox("Select core", options=core_opts, index=0, key="nl_pos_core_pick")
        try:
            _stats_tmp = get_stream_stats_cached(core_pick, int(cfg.window_days), df_all=df_all, last_date=last_all)
            _pos_tmp = get_pos_map_cached(core_pick, int(cfg.window_days), _stats_tmp, last_date=last_all)
        except Exception as e:
            st.error(f"Could not load percentile table for core {core_pick}: {e}")
            _pos_tmp = None

        if _pos_tmp is None or (hasattr(_pos_tmp, "empty") and _pos_tmp.empty):
            st.info("No percentile table available for this core/window.")
        else:
            st.dataframe(_pos_tmp, use_container_width=True, hide_index=True)
            try:
                _csv = _pos_tmp.to_csv(index=False).encode("utf-8")
                st.download_button(
                    "Download percentile table CSV",
                    data=_csv,
                    file_name=f"nl_position_percentiles_{core_pick}_{int(cfg.window_days)}d.csv",
                    mime="text/csv",
                )
            except Exception:
                pass
...
with tabs[1]:
        ...
        # Optional: show a per-core tab for every selected core (useful when budget is tight)
        show_core_tabs = st.checkbox(
            "Show tabs for all selected cores (optional)",
            value=False,
            key="show_core_tabs",
            help="If enabled, you'll get one Core View tab per core selected above. Useful for quickly comparing core tie-break percentile maps.",
        )

        selected_core_list = st.session_state.get("cores_for_cache") or [default_core]
        selected_core_list = [str(c).zfill(3) for c in selected_core_list if str(c).zfill(3) in cores]
        if not selected_core_list:
            selected_core_list = [default_core]

        def _render_one_core(core_for_view: str) -> None:
            st.markdown(f"### Core {core_for_view}")

            # Compute stream stats for this core (core ranking differs from the Northern Star master ranking)
            stream_stats = compute_stream_stats(df_all, core_for_view, cfg.window_days, exclude_md=exclude_md)

            if stream_stats.empty:
                st.info("No hits found for this core in the current data window.")
                return

            st.markdown("#### Stream ranking (AABC doubles)")
            st.dataframe(
                stream_stats[["RankPos","Stream","HitsWindow","HitsPerWeek","DaysSinceLastHit"]],
                use_container_width=True,
                hide_index=True,
            )

            # Core ranking percentile map (tie-breaker) — per selected core
            with st.expander("Core ranking percentile map (tie-breaker)", expanded=True):
                pos_map = get_position_percentiles_cached(core_for_view, cfg.window_days, stream_stats)
                st.caption("This is the percentile map for the **core ranking** (not the Northern Star master list). Use it as a tie-breaker when budget is tight.")
                st.dataframe(pos_map, use_container_width=True, hide_index=True)

            # Buckets (Top/Due/Combined) for this core
            st.markdown("#### Northern Star buckets")
            top_bucket, due_bucket, combined_bucket = compute_buckets(stream_stats)

            c1, c2, c3 = st.columns(3)
            with c1:
                st.markdown("**Top 12 (BaseScore)**")
                st.write(top_bucket)
            with c2:
                st.markdown("**Due 8 (DueIndex)**")
                st.write(due_bucket)
            with c3:
                st.markdown("**Combined (Top+Due)**")
                st.write(combined_bucket)

        if show_core_tabs and len(selected_core_list) > 1:
            core_tabs = st.tabs([f"Core {c}" for c in selected_core_list])
            for _t, _core in zip(core_tabs, selected_core_list):
                with _t:
                    _render_one_core(_core)
        else:
            core_for_view = st.selectbox("Choose a core", options=cores, index=cores.index(default_core), key="core_view_choose")
            _render_one_core(core_for_view)
    ...
    if enable_bt:
        ...
        if len(dates_all) < 3:
            ...
            def _build_plays_for_training(df_train: pd.DataFrame, last_train_date: pd.Timestamp) -> pd.DataFrame:
                ...
                selected_cores = [view_core] if view_core != "ALL" else (st.session_state.get("cores_for_cache") or [default_core])
...
for core, tab in zip(cores, tabs[2:]):
    with tab:
        ...
        if not pos_map.empty and pos_map["HitCount"].sum() > 0:
            capture = pos_map[pos_map["RankPos"].isin(_ensure_list(top30_positions))]["HitCount"].sum() / pos_map["HitCount"].sum() * 100.0
        ...
        with left:
            ...
            # --- Core ranking percentile map (per selected core) ---
            # Separate from the Northern Lights (master playlist) percentile tables.
            # This map is derived from the core's RankPos distribution and updates when inputs change.
            with st.expander("Core ranking: percentile map (per selected core)", expanded=False):
                _core_pos_map = get_position_percentiles_cached(core, int(window_days), stats)
                st.dataframe(_core_pos_map, use_container_width=True, hide_index=True)
