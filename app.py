...
APP_VERSION = "v51.10 (Fast Exact Backtest + Reports)"
...
import numpy as np
import pandas as pd
from ns_fast_backtest import render_fast_walk_forward_backtest
...
def _render_backtest_walk_forward(df_all: pd.DataFrame, cfg: "RankConfig", cores_for_cache: list[str]) -> None:
    """Optimized exact walk-forward backtest.

    The production app outside this function is unchanged. This replacement preserves
    the historical backtest contract (Base Top-N + Due bucket and member Top1/Top2)
    while moving the date/core/stream ledger generation to Polars.
    """
    render_fast_walk_forward_backtest(
        df_all=df_all,
        cfg=cfg,
        cores_for_cache=cores_for_cache,
        core_presets=CORE_PRESETS,
        members_from_core=members_from_core,
        box_key=box_key,
        core_member_label=core_member_label,
        predict_core_member=predict_core_member,
    )
