...
def engine_cluster_positions(
    ...
    Backward compatible:
      - If you pass a list of positions, we format it.
      - If you pass (df_24h_core_hits, base_stats), we build a RankPos/HitCount map first.
    """
    # Determine label
    if label is None:
    ...
    if isinstance(top_positions_or_df, (list, tuple, set)):
        ...
        if "Stream" not in df_24h_core_hits.columns or "Stream" not in base_stats.columns:
            return f"{label}: (missing Stream column)"
        ...
        if "RankPos" in base_stats.columns:
            ...
            return f"{label}: (missing RankPos column)"
        ...
        if pos_map.empty:
            return f"{label}: (no overlap with baseline ranks)"

        top_positions = top_dense_positions(pos_map.rename(columns={"RankPos": "RankPos"}), top_n=int(top_n))
