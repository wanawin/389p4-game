...
APP_VERSION = "v51.10 (12-Core WF Reports)"
...
OLD_APP_CORE_SET = ['016', '017', '018', '019', '023', '024', '025', '027', '028', '029', '038', '046', '048', '056', '059', '067', '068', '078', '129', '135', '138', '145', '146', '149', '167', '168', '169', '179', '236', '238', '239', '245', '246', '249', '256', '257', '258', '278', '279', '345', '348', '357', '359', '378', '379', '389', '457', '459', '489', '567', '579', '589', '679', '689', '789']
# Intended 12-core research set used by the walk-forward report build.
WF12_CORE_SET = ["016", "027", "028", "067", "138", "145", "256", "389", "457", "458", "567", "679"]
...
if st.button("Select intended 12 cores", key="select_wf12_cores"):
    st.session_state["cores_for_cache_ms"] = [c for c in WF12_CORE_SET if c in available_cores]
    _rerun()
...
def _render_backtest_walk_forward(df_all: pd.DataFrame, cfg: "RankConfig", cores_for_cache: list[str]) -> None:
    ...
    # Full candidate ledger: every test date x selected core x ranked stream.
    # This is separate from `rows`, which contains only actual winner rows.
    ...
    for test_date in dates_to_test:
        ...
        # Identify the actual selected-core winner(s) by stream for this date.
        actual_by_pair: dict[tuple[str, str], list[str]] = {}
        for _, _wr in day_winners.iterrows():
            _stream = str(_wr.get("Stream", "")).strip()
            _winner = str(_wr.get("Result", "")).strip()
            for _core in member_to_cores.get(box_key(_winner), []):
                actual_by_pair.setdefault((str(_core), _stream), []).append(_winner)

        # Export the complete per-core Northern Star ranking for every tested date.
        for _core in cores_sel:
            _key = (_core, test_date, int(cfg.window_days))
            if _key not in per_core_cache:
                _stats = compute_stream_stats(train_df, _core, window_days=int(cfg.window_days))
                per_core_cache[_key] = _stats
                per_core_buckets[_key] = bucket_recommendations(_stats, cfg)
            else:
                _stats = per_core_cache[_key]

            if _stats is None or _stats.empty:
                continue

            _buckets = per_core_buckets[_key]
            _base = set(_buckets.get("Top12BaseScore", pd.DataFrame()).get("Stream", pd.Series(dtype=str)).astype(str).tolist())
            _due = set(_buckets.get("Due8", pd.DataFrame()).get("Stream", pd.Series(dtype=str)).astype(str).tolist())

            for _, _sr in _stats.iterrows():
                _s = str(_sr.get("Stream", "")).strip()
                _wins = actual_by_pair.get((str(_core), _s), [])
                _bucket = "Both" if (_s in _base and _s in _due) else ("BaseScore" if _s in _base else ("Due8" if _s in _due else "None"))
                _row = {
                    "Date": test_date.date(),
                    "AsOfMaxDate": pd.to_datetime(train_df["Date"], errors="coerce").max().date(),
                    "Core": str(_core).zfill(3),
                    "Stream": _s,
                    "ExactStreamCoreHit": bool(_wins),
                    "Winner": "|".join(_wins),
                    "Bucket": _bucket,
                }
                for _c in [
                    "RankPos", "BaseScoreRank", "BaseScore", "HitsWindow", "DrawsWindow",
                    "HitsPerWeek", "LastHitDate", "DaysSinceLastHit", "DueIndex", "DueIndexRank",
                ]:
                    _v = _sr.get(_c, None)
                    _row[_c] = _v
                all_rank_rows.append(_row)
    ...
    all_ranks = pd.DataFrame(all_rank_rows)
    if not all_ranks.empty:
        for _c in ["RankPos", "BaseScoreRank"]:
            if _c in all_ranks.columns:
                all_ranks[_c] = pd.to_numeric(all_ranks[_c], errors="coerce").astype("Int64")
        all_ranks = all_ranks.sort_values(["Date", "Core", "RankPos", "Stream"], na_position="last")
    st.session_state["wf_all_rank_rows"] = all_ranks.copy()

    # Fixed row-cutoff audit. A cutoff of N means ranks 1..N for every selected core.
    cutoff_rows = []
    test_day_count = int(len(dates_to_test))
    for _rank_col in ["RankPos", "BaseScoreRank"]:
        if all_ranks.empty or _rank_col not in all_ranks.columns:
            continue
        for _n in [1, 2, 3, 4, 5, 10, 12, 20, 30, 40, 50]:
            _sub = all_ranks[pd.to_numeric(all_ranks[_rank_col], errors="coerce") <= int(_n)].copy()
            _hit = _sub[_sub["ExactStreamCoreHit"].fillna(False).astype(bool)]
            _days = int(_hit["Date"].nunique())
            cutoff_rows.append({
                "RankSystem": _rank_col,
                "RowsPerCore": int(_n),
                "SelectedCores": int(len(cores_sel)),
                "StreamCoreRowsPerDay": int(len(cores_sel) * _n),
                "All3MemberPlaysPerDay": int(len(cores_sel) * _n * 3),
                "DaysTested": test_day_count,
                "DaysWithAtLeast1ExactCoreStreamHit": _days,
                "DayHitPct": round((_days / max(1, test_day_count)) * 100.0, 2),
                "ExactWinnerEventsCaptured": int(len(_hit)),
            })
    cutoff_df = pd.DataFrame(cutoff_rows)

    day_summary = pd.DataFrame({"Date": [pd.Timestamp(d).date() for d in dates_to_test]})
    if not all_ranks.empty:
        _actual = all_ranks[all_ranks["ExactStreamCoreHit"].fillna(False).astype(bool)]
        _cnt = _actual.groupby("Date").size().rename("SelectedCoreWinnerEvents").reset_index()
        day_summary = day_summary.merge(_cnt, on="Date", how="left")
    day_summary["SelectedCoreWinnerEvents"] = day_summary.get("SelectedCoreWinnerEvents", 0)
    day_summary["SelectedCoreWinnerEvents"] = pd.to_numeric(day_summary["SelectedCoreWinnerEvents"], errors="coerce").fillna(0).astype(int)
    ...
    st.markdown("#### Walk-forward report downloads")
    st.caption(
        "Winner rows and the complete daily per-core rankings are separate. "
        "The complete ledger includes every tested date × selected core × stream, including misses."
    )

    _cfg_report = pd.DataFrame([{
        "APP_VERSION": APP_VERSION,
        "WindowDays": int(cfg.window_days),
        "StartDate": str(start_dt.date()),
        "EndDate": str(end_dt.date()),
        "MaxDates": int(max_dates),
        "OnlySelectedCoreHitDays": bool(only_hit_days),
        "MemberTracking": bool(member_track),
        "MemberBasis": str(member_basis),
        "IncludeRare": bool(include_rare),
        "SelectedCores": ",".join(str(c).zfill(3) for c in cores_sel),
        "SelectedCoreCount": int(len(cores_sel)),
    }])

    _d1, _d2, _d3 = st.columns(3)
    with _d1:
        st.download_button(
            "Download winner ledger",
            data=out.to_csv(index=False).encode("utf-8"),
            file_name="WF_WINNERS.csv",
            mime="text/csv",
            key="dl_wf_winners_v5110",
        )
    with _d2:
        st.download_button(
            "Download all daily ranks",
            data=all_ranks.to_csv(index=False).encode("utf-8"),
            file_name="WF_ALL_RANKS.csv",
            mime="text/csv",
            key="dl_wf_all_ranks_v5110",
        )
    with _d3:
        st.download_button(
            "Download cutoff audit",
            data=cutoff_df.to_csv(index=False).encode("utf-8"),
            file_name="WF_CUTOFFS.csv",
            mime="text/csv",
            key="dl_wf_cutoffs_v5110",
        )

    # One compact ZIP containing every report.
    try:
        import io as _io
        import zipfile as _zipfile
        _buf = _io.BytesIO()
        with _zipfile.ZipFile(_buf, "w", compression=_zipfile.ZIP_DEFLATED) as _zf:
            _zf.writestr("00_CONFIG.csv", _cfg_report.to_csv(index=False))
            _zf.writestr("01_WINNER_LEDGER.csv", out.to_csv(index=False))
            _zf.writestr("02_ALL_DAILY_RANKS.csv", all_ranks.to_csv(index=False))
            _zf.writestr("03_FIXED_CUTOFF_AUDIT.csv", cutoff_df.to_csv(index=False))
            _zf.writestr("04_DAY_SUMMARY.csv", day_summary.to_csv(index=False))
        st.download_button(
            "Download ALL WF reports (ZIP)",
            data=_buf.getvalue(),
            file_name="WF_REPORTS.zip",
            mime="application/zip",
            key="dl_wf_reports_zip_v5110",
        )
    except Exception as _zip_err:
        st.warning(f"Could not build report ZIP: {_zip_err}")

    if not cutoff_df.empty:
        st.markdown("##### Fixed row-cutoff audit")
    ...
    if member_track and (not out.empty):
        ...
        if missing:
            ...
            if member_df.empty:
                ...
                st.caption("The member predictor outputs only Top1 and Top2. The core still has exactly three boxed members; the unlisted member is not assigned a third prediction rank.")
