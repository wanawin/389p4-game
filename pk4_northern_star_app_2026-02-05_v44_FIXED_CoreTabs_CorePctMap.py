...
if "cores_for_cache" not in st.session_state or not st.session_state.cores_for_cache:
    st.session_state.cores_for_cache = [default_core]
if vc not in [str(c).zfill(3) for c in st.session_state.cores_for_cache]:
    st.session_state.cores_for_cache = [str(c).zfill(3) for c in st.session_state.cores_for_cache] + [vc]
# Also update widget state if it already exists
if "cores_for_cache_ms" in st.session_state:
    ms = [str(c).zfill(3) for c in (st.session_state.cores_for_cache_ms or [])]
    if vc not in ms:
        st.session_state.cores_for_cache_ms = ms + [vc]

# Multi-core selection for cache/batch tools
cores_for_cache = st.multiselect(
    "Cores to include for cache building / batch tools",
    options=available_cores,
    default=st.session_state.cores_for_cache,
...
with tabs[1]:
    ...
    # Optional: render per-core sub-tabs for *all* selected cores (the same cores used for cache building).
    show_core_tabs = st.checkbox(
        "Show tabs for all selected cores (optional)",
        value=False,
        key="core_view_show_tabs",
        help="When ON, you will get a separate sub-tab for each core selected in 'Cores to include for cache building / batch tools'."
    )

    # Core list for tabs comes from the multiselect near the top of the page.
    cores_for_tabs = list(dict.fromkeys(cores_mult)) if isinstance(cores_mult, (list, tuple)) else []

    def _render_single_core(core_id: str) -> None:
        df_core = get_stream_stats_cached(
            core_id,
        ...
        if df_core.empty:
            st.warning("No hits found for this core in the selected window.")
            return

        # Core percentile map (tie-breaker)
        with st.expander("Core ranking percentile map (tie-breaker)", expanded=False):
            pos_map = get_position_percentiles_cached(core_id, window_days, df_core)
            st.caption(
                "This is **core-only** positional strength for the current window. Use it as a tie-breaker when budget is tight (it does NOT change the Northern Star score)."
            )
        ...
        st.dataframe(
            df_core[["RankPos", "Stream", "HitsWindow", "HitsPerWeek", "DaysSinceLastHit"]],
            use_container_width=True,
            hide_index=True,
        )

        # Buckets
        buckets = bucket_recommendations(df_core, cfg)
        st.markdown("#### Northern Star buckets")
        col1, col2, col3 = st.columns(3)
        with col1:
            st.caption("Top 12 (BaseScore)")
            st.code("\n".join(_streams) if _streams else "(none)")
        with col2:
            st.caption("Due 8 (DueIndex)")
            st.code("\n".join(_streams) if _streams else "(none)")
        with col3:
            st.caption("Combined (Top+Due)")
            st.code("\n".join(_streams) if _streams else "(none)")

    if show_core_tabs and cores_for_tabs:
        subtabs = st.tabs([f"Core {c}" for c in cores_for_tabs])
        for tab, cid in zip(subtabs, cores_for_tabs):
            with tab:
                _render_single_core(cid)
    else:
        core_id = st.selectbox("Choose a core", CORE_PRESETS, index=CORE_PRESETS.index(core_for_view))
        _render_single_core(core_id)
