...
def _extract_available_cores(df: pd.DataFrame) -> List[str]:
    # Prefer explicit columns if present; otherwise infer from any 4-digit "Result" values.
    candidates: List[str] = []
    for col in ["Core", "core", "CORE", "CoreKey", "core_key", "Family", "family", "FAMILY"]:
        if col in df.columns:
            try:
                vals = df[col].dropna().astype(str).str.strip()
                for v in vals.unique().tolist():
                    if v.isdigit() and len(v) >= 3:
                        candidates.append(v[-3:].zfill(3))
            except Exception:
                pass

    if not candidates and "Result" in df.columns:
        try:
            vals = df["Result"].dropna().astype(str).str.replace(r"\D", "", regex=True)
            # Result might be 4 digits; use first 3 as core.
            for v in vals.unique().tolist():
    ...
    # Include any preset cores if defined
    try:
        if "CORE_PRESETS" in globals() and isinstance(CORE_PRESETS, dict):
            candidates.extend([str(k).zfill(3) for k in CORE_PRESETS.keys()])
    except Exception:
        pass
...
# Build list of available cores from data (and presets)
if df_all is not None and isinstance(df_all, pd.DataFrame) and len(df_all) > 0:
    available_cores = _extract_available_cores(df_all)
else:
    available_cores = []
    try:
        if "CORE_PRESETS" in globals() and isinstance(CORE_PRESETS, dict):
            available_cores = [str(k).zfill(3) for k in CORE_PRESETS.keys()]
    except Exception:
        available_cores = []
...
# Default core: prefer cfg.core_default if present; else first option
try:
    default_core = str(getattr(cfg, "core_default", getattr(cfg, "default_core", "389"))).zfill(3)
except Exception:
    default_core = "389"

if default_core not in available_cores:
    default_core = available_cores[0]

view_core = st.sidebar.selectbox(
    "View core (dropdown)",
...
# Multicore list used for cache building / batch tools.
# Keep user selections stable across reruns and always include current view_core.
prior = st.session_state.get("cores_for_cache", [])
if not isinstance(prior, list):
    prior = []
prior = [str(x).zfill(3) for x in prior if str(x).strip()]

if view_core not in prior:
    prior = [view_core] + prior

cores_for_cache = st.sidebar.multiselect(
    "Cores to include for cache building / batch tools",
    options=available_cores,
    default=[c for c in prior if c in available_cores],
    key="cores_for_cache",
)

st.sidebar.checkbox("Show tabs for all selected cores (can be slower)", key="show_all_selected_core_tabs", value=False)
