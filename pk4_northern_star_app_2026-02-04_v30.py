...
# pk4_northern_star_app_2026-02-03_v29.py
...
APP_VERSION = "v29 (2026-02-04) — label-only fix (version header)"
import re
...
def engine_cluster_positions(
    ...
    *,
    top_n: int = 10,
    rank_col: str = "RankPos_full",
) -> list[str]:
    """
    Build a "cluster" list of dense positions from the 24h core-hits table using base_stats RankPos.

    Robust to:
      - missing/empty inputs
      - NaN rank values
      - alternate rank column names ("RankPos", "RankPos_full")
    """
    if df_24h_core_hits is None or df_24h_core_hits.empty:
        return []

    if base_stats is None or not isinstance(base_stats, pd.DataFrame) or base_stats.empty:
        return []

    # Determine which rank column we actually have
    rank_candidates = [rank_col, "RankPos_full", "RankPos"]
    use_rank_col = next((c for c in rank_candidates if c in base_stats.columns), None)
    if use_rank_col is None:
        return []

    # Make a clean rank map: Stream -> RankPos (int)
    rank_map = base_stats[["Stream", use_rank_col]].copy()
    rank_map[use_rank_col] = pd.to_numeric(rank_map[use_rank_col], errors="coerce")
    rank_map = rank_map.dropna(subset=[use_rank_col])
    if rank_map.empty:
        return []
    rank_map["RankPos"] = rank_map[use_rank_col].astype(int)
    rank_map = rank_map[["Stream", "RankPos"]]

    dfh = df_24h_core_hits.copy()
    if "Stream" not in dfh.columns:
        return []
    dfh["Stream"] = dfh["Stream"].astype(str)

    # Attach RankPos to the 24h hits rows
    merged = dfh.merge(rank_map.assign(Stream=rank_map["Stream"].astype(str)), on="Stream", how="left")
    merged = merged.dropna(subset=["RankPos"])
    if merged.empty:
        return []

    # Prefer streams with higher 24h hits; if missing, just use order
    if "HitsWindow" in merged.columns:
        merged["HitsWindow"] = pd.to_numeric(merged["HitsWindow"], errors="coerce").fillna(0.0)
        merged = merged.sort_values(["HitsWindow", "RankPos"], ascending=[False, True])
    else:
        merged = merged.sort_values(["RankPos"], ascending=[True])

    # Cluster = top_n unique RankPos values, then expand to nearby positions (±1)
    top_rankpos = merged["RankPos"].astype(int).tolist()[: max(int(top_n), 0)]
    if not top_rankpos:
        return []

    dense = set()
    for rp in top_rankpos:
        dense.add(int(rp))
        dense.add(int(rp) - 1)
        dense.add(int(rp) + 1)

    dense = [rp for rp in sorted(dense) if 1 <= rp <= 78]
    return [str(rp) for rp in dense]
...
with tabs[0]:
    ...
    with c2:
        nl_autorun = st.checkbox("Auto-run when inputs change", value=True, help="Runs automatically when you change controls above. If off, use the Run button.")
