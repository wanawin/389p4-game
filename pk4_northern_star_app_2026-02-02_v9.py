...
# pk4_northern_star_app_2026-02-02_v7.py
...
tab_labels = ["Northern Lights (Master playlist)", "Backtest (optional)"] + [f"Core {c}" for c in cores]
...
st.divider()
st.subheader("Final scored plays (member-level AABC)")
st.caption(
    "This expands each (core, stream) recommendation into its 3 AABC member box-keys and scores them using "
    "a universal, cross-core percentile score. No simulations — computed from your uploaded history."
)

df_window = within_last_days(df_all, cfg.window_days)

play_rows = []
for _, mrow in df_master.iterrows():
    core = str(mrow["Core"]).zfill(3)
    stream = mrow["Stream"]
    bucket = mrow["Bucket"]
    rankpos = int(mrow["RankPos"])
    pos_pct = float(mrow["PosHitCountPctile"]) if pd.notna(mrow["PosHitCountPctile"]) else np.nan

    for member in members_from_core(core, "AABC"):
        sub = df_window[(df_window["Stream"] == stream) & (df_window["Box"] == member)]
        hits = int(len(sub))
        last_hit = sub["Date"].max() if hits > 0 else pd.NaT
        days_since = (last_all - last_hit).days if hits > 0 and pd.notna(last_hit) else np.nan
        ...
        play_rows.append({
            ...
            "RankPos": rankpos,
            "MemberBox": member,
            "StreamHitsPerWeek": float(mrow["HitsPerWeek"]),
            "StreamDaysSinceLastHit": float(mrow["DaysSinceLastHit"]) if pd.notna(mrow["DaysSinceLastHit"]) else np.nan,
            "PosHitCountPctile": pos_pct,
            "MemberHitsWindow": hits,
...
if df_plays.empty:
    st.info("No member-level plays yet. Check that your history and core list loaded correctly.")
else:
    # percentiles across the full play list (universal comparison)
    df_plays["StreamHitsPerWeekPct"] = df_plays["StreamHitsPerWeek"].rank(pct=True) * 100.0
    df_plays["StreamDuePct"] = df_plays["StreamDaysSinceLastHit"].rank(pct=True) * 100.0
    df_plays["StreamDuePct"] = df_plays["StreamDuePct"].fillna(100.0)

    df_plays["MemberHitsPerWeekPct"] = df_plays["MemberHitsPerWeek"].rank(pct=True) * 100.0
    df_plays["MemberDuePct"] = df_plays["MemberDaysSinceLastHit"].rank(pct=True) * 100.0
    df_plays["MemberDuePct"] = df_plays["MemberDuePct"].fillna(100.0)

    # Fill missing pos pct with median (safe fallback)
    if df_plays["PosHitCountPctile"].notna().any():
        pos_fill = float(df_plays["PosHitCountPctile"].median())
    else:
        pos_fill = 50.0
    df_plays["PosPctFill"] = df_plays["PosHitCountPctile"].fillna(pos_fill)

    # Universal score weights (checkbox-testable later if desired)
    w_stream_hits, w_stream_due = 0.30, 0.15
    w_pos, w_member_hits, w_member_due = 0.20, 0.20, 0.15

    df_plays["FinalScore"] = (
        w_stream_hits * df_plays["StreamHitsPerWeekPct"] +
        w_stream_due  * df_plays["StreamDuePct"] +
        w_pos         * df_plays["PosPctFill"] +
        w_member_hits * df_plays["MemberHitsPerWeekPct"] +
        w_member_due  * df_plays["MemberDuePct"]
    ).round(2)

    df_plays = df_plays.sort_values(
        ["FinalScore", "StreamHitsPerWeek", "MemberHitsPerWeek", "MemberDuePct"],
        ascending=False
    ).reset_index(drop=True)

    df_plays.insert(0, "FinalRank", np.arange(1, len(df_plays) + 1))

    st.dataframe(
        df_plays[[
            "FinalRank","FinalScore","Core","Stream","MemberBox","Bucket","RankPos",
            "StreamHitsPerWeek","StreamDaysSinceLastHit","PosHitCountPctile",
            "MemberHitsWindow","MemberHitsPerWeek","MemberDaysSinceLastHit"
        ]],
        use_container_width=True,
        hide_index=True
    )

    plays_csv = df_plays.to_csv(index=False).encode("utf-8")
    st.download_button(
        "Download Final Scored Plays CSV",
        data=plays_csv,
        file_name="pk4_northern_lights_final_scored_plays.csv",
        mime="text/csv",
    )
...
with tabs[1]:
    ...
    if enable_bt:
        ...
        if len(dates_all) < 3:
            ...
            max_d = pd.to_datetime(dates_all[-1]).date()
            min_d = pd.to_datetime(dates_all[0]).date()
            default_start = max(min_d, (max_d - pd.Timedelta(days=14)).date())
