...
# Ensure a safe default core exists (do not depend on cfg before it is created)
default_core = str(getattr(st.session_state, "default_core", "389")).zfill(3)
if default_core not in available_cores:
    available_cores = [default_core] + available_cores

# Persist selection (single source of truth = st.session_state["cores_for_cache"])
if "cores_for_cache" not in st.session_state:
    st.session_state["cores_for_cache"] = [default_core]

# Core view dropdown (single core)
_prior_view = st.session_state.get("view_core", default_core)
if _prior_view not in available_cores:
    _prior_view = default_core
view_core = st.selectbox("View core (dropdown)", options=available_cores, index=available_cores.index(_prior_view), key="view_core")
