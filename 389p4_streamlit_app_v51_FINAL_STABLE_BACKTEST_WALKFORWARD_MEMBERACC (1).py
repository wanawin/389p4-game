...
if member_track:
    fam = out[out["ActualFamilyMember"].notna()].copy()
    if not fam.empty:
        try:
            top1_rate = float(pd.to_numeric(fam["MemberHitTop1"], errors="coerce").mean())
            top2_rate = float(pd.to_numeric(fam["MemberHitTop2"], errors="coerce").mean())
        except Exception:
            top1_rate = None
            top2_rate = None

        cma, cmb, cmc = st.columns(3)
        cma.metric("Member Top1 accuracy", f"{(top1_rate*100):.1f}%" if top1_rate is not None else "n/a")
        cmb.metric("Member Top2 coverage", f"{(top2_rate*100):.1f}%" if top2_rate is not None else "n/a")
        cmc.metric("Member-evaluable wins", f"{len(fam):,}")

        st.markdown("#### Member accuracy by core (family only)")
        core_mem = fam.groupby("Core", as_index=False).agg(
            Wins=("ActualFamilyMember", "size"),
            Top1=("MemberHitTop1", "mean"),
            Top2=("MemberHitTop2", "mean"),
            TrainN=("MemberTrainN", "median"),
        )
        core_mem["Top1"] = core_mem["Top1"].astype(float)
        core_mem["Top2"] = core_mem["Top2"].astype(float)
        core_mem = core_mem.sort_values(["Top2", "Top1", "Wins"], ascending=[False, False, False]).reset_index(drop=True)
        st.dataframe(core_mem, use_container_width=True)
    else:
        st.info("No family-member wins were eligible for member accuracy (AABC/ABBC/ABCC) in the selected dates/window.")
    ...
    st.download_button(
        "Download Hit/Miss detail (CSV)",
        data=csv_bytes,
        file_name="walkforward_hit_miss_detail.csv",
        mime="text/csv",
        use_container_width=True,
    )
