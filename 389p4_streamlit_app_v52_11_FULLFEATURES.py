...
APP_VERSION = "v52.10_SELECTEDCORES_FIX_2026-02-08"

# --- Safety defaults to prevent NameError across tabs (Streamlit reruns) ---
DEFAULT_CORE = "389"
view_core = DEFAULT_CORE
selected_cores = []

BUILD_LABEL = "BUILD: v51.02 | 2026-02-09 03:40 UTC"
...
def position_percentile_map(df_rankpos: pd.DataFrame) -> pd.DataFrame:
    ...
    if df_rankpos is None or df_rankpos.empty:
        return pd.DataFrame(columns=[
            "RankPos", "HitCount", "HitCountPctile", "PctStrength", "HitSharePct", "CumuHitSharePct", "HitShare", "CumHitShare"
        ])
    ...
    # Back-compat alias: PctStrength matches HitCountPctile (0-100)
    pos["PctStrength"] = pos["HitCountPctile"].astype(float)
    ...
    if key in cache:
        try:
            ...
            if _pm is not None and not _pm.empty:
                if "PctStrength" not in _pm.columns and "HitCountPctile" in _pm.columns:
                    _pm["PctStrength"] = _pm["HitCountPctile"]
                if "HitShare" not in _pm.columns and "HitSharePct" in _pm.columns:
                    _pm["HitShare"] = _pm["HitSharePct"]
                if "CumHitShare" not in _pm.columns and "CumuHitSharePct" in _pm.columns:
                    _pm["CumHitShare"] = _pm["CumuHitSharePct"]
        except Exception:
            pass
        return cache[key]
...
def _load_baseline_from_disk(core: str, window_days: int, expected_last_date: str | None):
    ...
    # Back-compat: older cached percentile maps may not include alias columns
    try:
        if pos_df is not None and not pos_df.empty:
            if "PctStrength" not in pos_df.columns and "HitCountPctile" in pos_df.columns:
                pos_df["PctStrength"] = pos_df["HitCountPctile"]
            if "HitShare" not in pos_df.columns and "HitSharePct" in pos_df.columns:
                pos_df["HitShare"] = pos_df["HitSharePct"]
            if "CumHitShare" not in pos_df.columns and "CumuHitSharePct" in pos_df.columns:
                pos_df["CumHitShare"] = pos_df["CumuHitSharePct"]
    except Exception:
        pass
...
def get_pos_map_cached(core: str, window_days: int, stream_stats_df, last_date: str | None):
    ...
    # Back-compat: ensure alias columns exist (older cached maps)
    try:
        if pos_df is not None and not pos_df.empty:
            if "PctStrength" not in pos_df.columns and "HitCountPctile" in pos_df.columns:
                pos_df["PctStrength"] = pos_df["HitCountPctile"]
...
tab_labels = ["Northern Lights (Master playlist)", "Northern Star (v51)", "Core view", "Backtest (optional)"]
_t_nl, _t_ns, _t_core, _t_bt = st.tabs(tab_labels)
...
with _t_ns:
    ...
    if not ns_cores:
        ...
        try:
            ...
            global_map, missing = pd.DataFrame(), [f"ERROR: {_e}"]
            st.error("Global core percentile map failed to load/build. See details below.")
            st.exception(_e)
        if missing:
    ...
    st.subheader("Per-core Northern Star scoring (SeedTraits + Cadence, soft)")
        # Ensure the "selected_cores" list exists for this tab (it is stored in session_state by Core selection)
    _sel = st.session_state.get("cores_for_cache_ms") or st.session_state.get("selected_cores") or []
    selected_cores = list(_sel) if isinstance(_sel, (list, tuple, set)) else ([] if _sel is None else [_sel])
    # view_core is set in Core selection; keep a safe fallback in case the tab is opened before selection
    if not isinstance(view_core, str) or not view_core:
        view_core = (selected_cores[0] if selected_cores else DEFAULT_CORE)
    ...
    try:
        stats_ns = compute_stream_stats(core_key_ns, cfg)
    except Exception as e:
