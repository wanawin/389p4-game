...
APP_VERSION = "v42+NLpct+CoreTabsFix2"
...
try:
    ...
    vc = default_core
cores_for_cache = st.session_state.cores_for_cache

# Initialize multiselect state once (prevents default/session mismatches)
if "cores_for_cache_ms" not in st.session_state:
    st.session_state.cores_for_cache_ms = list(cores_for_cache)

# Sanitize initial selection (and always keep the default core)
_tmp_vals = [str(c).zfill(3) for c in st.session_state.get("cores_for_cache_ms", [])]
if not _tmp_vals:
    _tmp_vals = [default_core]
if default_core not in _tmp_vals:
    _tmp_vals = [default_core] + [v for v in _tmp_vals if v != default_core]
st.session_state.cores_for_cache_ms = _tmp_vals
st.session_state.cores_for_cache = _tmp_vals

def _sync_cores_for_cache_ms():
    vals = [str(c).zfill(3) for c in st.session_state.get("cores_for_cache_ms", [])]
    if not vals:
        vals = [default_core]
    if default_core not in vals:
        vals = [default_core] + [v for v in vals if v != default_core]
    st.session_state.cores_for_cache_ms = vals
    st.session_state.cores_for_cache = vals

cores_for_cache = st.multiselect(
    "Cores to include for cache building / batch tools",
    options=CORE_PRESETS,
    key="cores_for_cache_ms",
    on_change=_sync_cores_for_cache_ms,
)
cores_for_cache = list(st.session_state.get("cores_for_cache_ms", []))
...
with tabs[1]:
    ...
    if df_all is None or df_all.empty:
        ...
        with st.expander("Core ranking percentile map (tie-breaker)", expanded=False):
            # Percentile bands derived from RankPos only (display-only; does not change ranking)
            _dfp = stats[["RankPos", "Stream"]].copy()
            _n = int(len(_dfp)) if _dfp is not None else 0
            if _n == 0:
                st.info("No ranked streams available for this core.")
            else:
                _dfp["RankPct"] = (_dfp["RankPos"] / _n) * 100.0
                _bands = [
                    (0, 5, "Top 5%"),
                    (5, 20, "5–20%"),
                    (20, 50, "20–50%"),
                ...
                for lo, hi, label in _bands:
                    _sub = _dfp[(_dfp["RankPct"] > lo) & (_dfp["RankPct"] <= hi)].sort_values("RankPos")
                    _rows.append({
                        "Band": label,
                        "Percentile range": f"{lo}–{hi}%",
                        "Count": int(len(_sub)),
                        "Streams (in rank order)": " | ".join(_sub["Stream"].tolist()),
                    })
                st.dataframe(pd.DataFrame(_rows), use_container_width=True, height=260)
                st.caption("Tie-breaker view: lower bands = stronger within this core. This updates automatically when you change the core, window, or upload a new history file.")
...
# --- Per-core tabs ---
for core, tab in zip(cores_for_tabs, tabs[3:]):
    with tab:
