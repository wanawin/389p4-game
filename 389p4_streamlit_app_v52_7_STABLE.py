...
APP_VERSION = "v52.7_STABLE_2026-02-08"
...
def build_allcores_rankpos_pctmap(
    cores_list: List[str],
    window_days: int,
    expected_last_date: Optional[pd.Timestamp] = None,
    cache_only: bool = False,
) -> Tuple[Optional[pd.DataFrame], List[str]]:
    """Build an "all cores" position-percentile tie-breaker map.

    Returns:
      (pct_df, missing_cores)

    Notes:
      - When cache_only=True this function will NOT compute live. If any required
        core cache is missing/outdated it returns (None, missing_cores).
    """
    ...
    for core in cores_list:
        if cache_only:
            ss, _pos_df, _meta = _load_baseline_from_disk(
            ...
            ss = compute_stream_stats_for_core(df_all, core, window_days, exclude_md=False)

        # Ensure required columns exist before concatenation
        want_cols = ["stream", "N", "P1", "P2", "P3", "P4"]
        if ss is None or ss.empty or not all(c in ss.columns for c in want_cols):
            missing.append(core)
            continue

        frames.append(ss[want_cols].copy())

    # In strict cache-only mode, refuse partial builds
...
_t_ns = nullcontext()
with _t_ns:
    st.header(f"Northern Star ({APP_VERSION})")
    ...
    try:
        stats_ns = compute_stream_stats(df_all, core_key_ns, cfg.window_days)
    except Exception as e:
