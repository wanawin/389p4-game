...
map24_file = None  # backward-compatible alias set in sidebar
...
with st.sidebar:
    ...
    map24_file = map24_file  # backward-compatible alias
...
# NOTE:
# Streamlit widgets already persist their own state. The safest way to support multi-core
# is to use stable widget keys and only mutate session_state for those keys *before* widget
# creation (or inside widget callbacks).
...
# ---------- widget state init (BEFORE widgets) ----------
default_core = str(getattr(cfg, "default_core", "389")).zfill(3)

# persistent, non-widget store for the multiselect
if "cores_for_cache_store" not in st.session_state:
    st.session_state["cores_for_cache_store"] = [default_core]

# Keep the store valid (options may change when a new history file is uploaded)
store = [c for c in st.session_state["cores_for_cache_store"] if c in available_cores]
if default_core in available_cores and default_core not in store:
    store = [default_core] + store
if not store and available_cores:
    store = [available_cores[0]]
st.session_state["cores_for_cache_store"] = store
...
# Auto-add the viewed core into the cache/batch list without touching the widget key directly.
# (We update the store + clear the multiselect widget state so it re-initializes from default.)
if view_core and view_core not in st.session_state["cores_for_cache_store"]:
    st.session_state["cores_for_cache_store"] = st.session_state["cores_for_cache_store"] + [view_core]
    if "cores_for_cache_ms" in st.session_state:
        del st.session_state["cores_for_cache_ms"]

cores_for_cache_sel = st.multiselect(
    "Cores to include for cache building / batch tools",
    options=available_cores,
    default=st.session_state["cores_for_cache_store"],
    key="cores_for_cache_ms",
    help="Pick 1+ cores for batch tools (cache build + Northern Lights). This list is independent from the 'View core' dropdown.",
)

# sanitize selection (no empties, unique, preserve order)
out = []
seen = set()
for c in cores_for_cache_sel:
    c = str(c).zfill(3)
    if c and c in available_cores and c not in seen:
        out.append(c); seen.add(c)

# ensure default + view core always present
for must in [default_core, view_core]:
    if must and must in available_cores and must not in seen:
        out.append(must); seen.add(must)

st.session_state["cores_for_cache_store"] = out
cores_for_cache = out
...
with c2:
    ...
    cores = list(st.session_state.get('cores_for_cache_store', [str(view_core).zfill(3)]))
